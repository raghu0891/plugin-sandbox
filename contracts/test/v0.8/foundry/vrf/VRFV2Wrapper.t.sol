...
contract VRFV2PlusWrapperTest is BaseTest {
  ...
  function testSetLinkAndLinkNativeFeed() public {
    ...
    // Set PLI and PLI/Native feed on wrapper.
    wrapper.setPLI(address(s_linkToken));
    ...
    // Revert for subsequent assignment.
    vm.expectRevert(VRFV2PlusWrapperConsumerBase.PLIAlreadySet.selector);
    consumer.setLinkToken(address(s_linkToken));
  }
  ...
  function testRequestAndFulfillRandomWordsNativeWrapper() public {
    ...
    s_wrapper.withdrawNative(PLI_WHALE, paid);
  ...
  function testRequestAndFulfillRandomWordsPLIWrapper() public {
    ...
    s_wrapper.withdraw(PLI_WHALE, paid);
