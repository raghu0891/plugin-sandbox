...
describe('VRFSubscriptionBalanceMonitor', () => {
  beforeEach(async () => {
    ...
    // Transfer PLI to stranger.
    await lt.transfer(stranger.address, oneHundredPLI)
  ...
  describe('setWatchList() / getWatchList() / getAccountInfo()', () => {
    it('Should allow owner to set the watchlist', async () => {
      ...
      let setTx = await bm
        .connect(owner)
        .setWatchList([sub1], [onePLI], [twoPLI])
      await setTx.wait()
      ...
      expect(subInfo.minBalanceJuels).to.equal(onePLI)
      expect(subInfo.topUpAmountJuels).to.equal(twoPLI)
      ...
      expect(subInfo1.minBalanceJuels).to.equal(onePLI)
      expect(subInfo1.topUpAmountJuels).to.equal(twoPLI)
      expect(subInfo2.isActive).to.be.true
      expect(subInfo2.minBalanceJuels).to.equal(twoPLI)
      expect(subInfo2.topUpAmountJuels).to.equal(threePLI)
      expect(subInfo3.isActive).to.be.true
      expect(subInfo3.minBalanceJuels).to.equal(threePLI)
      expect(subInfo3.topUpAmountJuels).to.equal(fivePLI)
      // remove some from watchlist
      setTx = await bm
        .connect(owner)
        .setWatchList([sub3, sub1], [threePLI, onePLI], [fivePLI, twoPLI])
      await setTx.wait()
      watchList = await bm.getWatchList()
    ...
    it('Should revert if the list lengths differ', async () => {
      let tx = bm.connect(owner).setWatchList([sub1], [], [twoPLI])
      await expect(tx).to.be.revertedWith(INVALID_WATCHLIST_ERR)
      tx = bm.connect(owner).setWatchList([sub1], [onePLI], [])
      await expect(tx).to.be.revertedWith(INVALID_WATCHLIST_ERR)
    ...
    it('Should revert if any of the subIDs are zero', async () => {
      let tx = bm
        .connect(owner)
        .setWatchList([sub1, 0], [onePLI, onePLI], [twoPLI, twoPLI])
      await expect(tx).to.be.revertedWith(INVALID_WATCHLIST_ERR)
    })
    ...
    it('Should revert if any of the top up amounts are 0', async () => {
      const tx = bm
        .connect(owner)
        .setWatchList([sub1, sub2], [onePLI, onePLI], [twoPLI, zeroPLI])
      await expect(tx).to.be.revertedWith(INVALID_WATCHLIST_ERR)
    })
  })
  ...
  describe('performUpkeep()', () => {
    ...
    context('when partially funded', () => {
      it('Should fund as many subscriptions as possible', async () => {
        const fundTx = await lt.connect(owner).transfer(
          bm.address,
          fivePLI, // only enough PLI to fund 2 subscriptions
        )
        await fundTx.wait()
