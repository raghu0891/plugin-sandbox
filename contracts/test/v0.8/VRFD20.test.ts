...
describe('VRFD20', () => {
  ...
  describe('#withdrawPLI', () => {
    describe('failure', () => {
      it('reverts when called by a non-owner', async () => {
        await expect(
          vrfD20
            .connect(roles.stranger)
            .withdrawPLI(await roles.stranger.getAddress(), deposit),
        ).to.be.revertedWith('Only callable by owner')
      })

      it('reverts when not enough PLI in the contract', async () => {
    ...
    describe('success', () => {
      it('withdraws PLI', async () => {
        ...
        await vrfD20
          .connect(roles.defaultAccount)
          .withdrawPLI(await roles.defaultAccount.getAddress(), deposit)
  ...
  describe('#rollDice', () => {
    ...
    describe('failure', () => {
      it('reverts when PLI balance is zero', async () => {
        const vrfD202 = await vrfD20Factory
          .connect(roles.defaultAccount)
          .deploy(vrfCoordinator.address, link.address, keyHash, fee)
        await expect(
          vrfD202.rollDice(await personas.Nancy.getAddress()),
        ).to.be.revertedWith('Not enough PLI to pay fee')
      })
