 ...
 */
contract AutomationRegistrar2_2 is TypeAndVersionInterface, ConfirmedOwner, IERC677Receiver {
  /**
  ...
  event ConfigChanged(address AutomationRegistry, uint96 minPLIJuels);
  ...
  constructor(
    ...
    setConfig(AutomationRegistry, minPLIJuels);
   ...
   * @param minPLIJuels minimum PLI that new registrations should fund their upkeep with
   */
  function setConfig(address AutomationRegistry, uint96 minPLIJuels) public onlyOwner {
    s_config = RegistrarConfig({
      minPLIJuels: minPLIJuels,
      AutomationRegistry: IAutomationRegistryMaster(AutomationRegistry)
    });
    emit ConfigChanged(AutomationRegistry, minPLIJuels);
  }
   ...
   */
  function getConfig() external view returns (address AutomationRegistry, uint256 minPLIJuels) {
    RegistrarConfig memory config = s_config;
    return (address(config.AutomationRegistry), config.minPLIJuels);
  }
  ...
  function _approve(RegistrationParams memory params, bytes32 hash) private returns (uint256) {
    ...
    bool success = PLI.transferAndCall(address(AutomationRegistry), params.amount, abi.encode(upkeepId));
    if (!success) {
