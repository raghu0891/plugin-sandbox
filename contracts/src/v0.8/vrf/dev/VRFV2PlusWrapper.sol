...
contract VRFV2PlusWrapper is ConfirmedOwner, TypeAndVersionInterface, VRFConsumerBaseV2Plus, IVRFV2PlusWrapper {
   ...
   */
  function setPLI(address link) external onlyOwner {
   ...
   *
   * @param _fallbackWeiPerUnitLink is the backup PLI exchange rate used when the PLI/NATIVE feed
   *        is stale.
   *
   * @param _fulfillmentFlatFeeLinkPPM is the flat fee in millionths of PLI that VRFCoordinatorV2Plus
   *        charges.
   *
   ...
   *         and fallback to fallbackWeiPerUnitLink.
   *
   * @return fulfillmentFlatFeeLinkPPM is the flat fee in millionths of PLI that VRFCoordinatorV2Plus
   *         charges.
   *
  ...
  function onTokenTransfer(address _sender, uint256 _amount, bytes calldata _data) external onlyConfiguredNotDisabled {
    ...
    require(msg.sender == address(s_link), "only callable from PLI");
