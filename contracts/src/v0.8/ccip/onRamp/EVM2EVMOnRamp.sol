...
contract EVM2EVMOnRamp is IEVM2AnyOnRamp, ILinkAvailable, AggregateRateLimiter, ITypeAndVersion {
  ...
  /// @dev The amount of PLI available to pay NOPS
  ...
  /// @notice Get the total amount of fees to be paid to the Nops (in PLI)
  /// @return totalNopFees
  ...
  function _setNops(NopAndWeight[] memory nopsAndWeights) internal {
    ...
    for (uint256 i = 0; i < numberOfNops; ++i) {
      // Make sure the PLI token is not a nop because the link token doesn't allow
      // self transfers. If set as nop, payNops would always revert. Since setNops
      // calls payNops, we can never remove the PLI token as a nop.
  ...
  /// @notice Calculate remaining PLI balance after paying nops
  ...
  function linkAvailableForPayment() public view returns (int256) {
    // Since PLI caps at uint96, casting to int256 is safe
