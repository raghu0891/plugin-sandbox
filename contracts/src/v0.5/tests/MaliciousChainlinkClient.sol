...
contract MaliciousPluginClient is PluginClient {
    ...
    returns(bytes32 requestId)
  {
    ...
    require(_link.transferAndCall(pluginOracleAddress(), _amount, encodeTargetRequest(_req)), "Unable to transferAndCall to oracle");
    maliciousRequests += 1;
    ...
    returns(bytes32 requestId)
  {
    ...
    emit PluginRequested(requestId);
    LinkTokenInterface _link = LinkTokenInterface(pluginTokenAddress());
    require(_link.transferAndCall(pluginOracleAddress(), _amount, encodePriceRequest(_req)), "Unable to transferAndCall to oracle");
    maliciousRequests += 1;
    ...
    returns(bytes32 requestId)
  {
    ...
    emit PluginRequested(requestId);
    LinkTokenInterface _link = LinkTokenInterface(pluginTokenAddress());
    require(_link.transferAndCall(pluginOracleAddress(), _wei, encodeWithdrawRequest(_req)), "Unable to transferAndCall to oracle");
    maliciousRequests += 1;
    return requestId;
  }
