 ...
 */
contract Oracle is PluginRequestInterface, OracleInterface, Ownable {
   ...
   * @param _link The address of the PLI token
   */
  ...
  function onTokenTransfer(
    ...
    permittedFunctionsForPLI(_data)
  {
   ...
   */
  modifier onlyPLI() {
    require(msg.sender == address(LinkToken), "Must use PLI token");
    _;
  }
   ...
   */
  modifier permittedFunctionsForPLI(bytes _data) {
    bytes4 funcSelector;
    assembly { // solhint-disable-line no-inline-assembly
  ...
  modifier checkCallbackAddress(address _to) {
    require(_to != address(LinkToken), "Cannot callback to PLI");
    _;
  }
