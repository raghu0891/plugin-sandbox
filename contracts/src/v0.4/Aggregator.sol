...
contract Aggregator is AggregatorInterface, PluginClient, Ownable {
  ...
  function transferPLI(address _recipient, uint256 _amount)
    ...
    onlyOwner()
  {
    LinkTokenInterface linkToken = LinkTokenInterface(pluginTokenAddress());
    require(linkToken.transfer(_recipient, _amount), "PLI transfer failed");
  }
  ...
  {
    LinkTokenInterface linkToken = LinkTokenInterface(pluginTokenAddress());
    transferPLI(owner, linkToken.balanceOf(address(this)));
    selfdestruct(owner);
  }
