...
import (
	...
	"github.com/goplugin/pluginv3.0/v2/core/web/resolver"
...
func NewRouter(app plugin.Application, prometheus *ginprom.Prometheus) (*gin.Engine, error) {
	engine := gin.New()
	...
	engine.Use(
		otelgin.Middleware("plugin-web-routes"),
...
func graphqlHandler(app plugin.Application) gin.HandlerFunc {
...
func debugRoutes(app plugin.Application, r *gin.RouterGroup) {
...
func sessionRoutes(app plugin.Application, r *gin.RouterGroup) {
	config := app.GetConfig()
	rl := config.WebServer().RateLimit()
...
func healthRoutes(app plugin.Application, r *gin.RouterGroup) {
...
func loopRoutes(app plugin.Application, r *gin.RouterGroup) {
...
func v2Routes(app plugin.Application, r *gin.RouterGroup) {
	...
	))
	{
		...
		for _, chain := range []struct {
			...
			if chain.path == "evm" {
				// TODO still EVM only https://app.shortcut.com/pluginlabs/story/26276/multi-chain-type-ui-node-chain-configuration
