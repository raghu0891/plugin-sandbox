...
func getOCR2Spec100() OffchainReporting2OracleSpec100 {
	return OffchainReporting2OracleSpec100{
		...
		MonitoringEndpoint:                null.StringFrom("endpoint:plugin.monitor"),
...
func TestSetMigrationENVVars(t *testing.T) {
	t.Run("ValidEVMConfig", func(t *testing.T) {
		...
		testConfig := configtest.NewGeneralConfig(t, func(c *plugin.Config, s *plugin.Secrets) {
			evmEnabled := true
	...
	t.Run("EVMConfigMissing", func(t *testing.T) {
		chainID := ubig.New(big.NewInt(1337))
		testConfig := configtest.NewGeneralConfig(t, func(c *plugin.Config, s *plugin.Secrets) { c.EVM = nil })
