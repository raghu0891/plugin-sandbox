...
import (
	...
	"github.com/goplugin/pluginv3.0/v2/core/store/migrate/migrations" // Invoke init() functions within migrations pkg.
)
...
func ensureMigrated(ctx context.Context, db *sql.DB, p *goose.Provider, providerTableName string) error {
	...
	if !found {
		return pkgerrors.New("database state is too old. Need to migrate to plugin version 0.9.10 first before upgrading to this version. This upgrade is NOT REVERSIBLE, so it is STRONGLY RECOMMENDED that you take a database backup before continuing")
	}
...
func SetMigrationENVVars(generalConfig plugin.GeneralConfig) error {
