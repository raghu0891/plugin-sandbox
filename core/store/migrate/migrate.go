...
func ensureMigrated(ctx context.Context, db *sql.DB, lggr logger.Logger) error {
	...
	if !found {
		return errors.New("database state is too old. Need to migrate to plugin version 0.9.10 first before upgrading to this version. This upgrade is NOT REVERSIBLE, so it is STRONGLY RECOMMENDED that you take a database backup before continuing")
	}
