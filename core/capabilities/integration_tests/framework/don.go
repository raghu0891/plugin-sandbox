...
import (
	...
	kcr "github.com/goplugin/pluginv3.0/v2/core/gethwrappers/keystone/generated/capabilities_registry"
...
type DON struct {
	...
	nodeConfigModifiers []func(c *plugin.Config, node *capabilityNode)
	...
	for i, member := range donConfig.Members {
		...
		cn.start = func() {
			node := startNewNode(ctx, t, lggr.Named(donConfig.name+"-"+strconv.Itoa(i)), nodeInfo, donContext.EthBlockchain,
				...
				donConfig.keys[i], func(c *plugin.Config) {
...
func (d *DON) AddEthereumWriteTargetNonStandardCapability(forwarderAddr common.Address) error {
	d.nodeConfigModifiers = append(d.nodeConfigModifiers, func(c *plugin.Config, node *capabilityNode) {
