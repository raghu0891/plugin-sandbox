...
type MercuryV02Response struct {
	PluginBlob string `json:"pluginBlob"`
}
...
func (c *client) singleFeedRequest(ctx context.Context, ch chan<- mercury.MercuryData, index int, sl *mercury.StreamsLookup) {
	...
	retryErr := retry.Do(
		func() error {
			...
			if blobBytes, err = hexutil.Decode(m.PluginBlob); err != nil {
				c.lggr.Warnf("at block %s upkeep %s failed to decode pluginBlob %s for feed %s: %v", sl.Time.String(), sl.UpkeepId.String(), m.PluginBlob, sl.Feeds[index], err)
				ch <- mercury.MercuryData{
					Index:   index,
					Bytes:   nil,
					ErrCode: encoding.ErrCodeStreamsBadResponse,
