...
func TestCoreRelayerChainInteroperators(t *testing.T) {
	...
	tests := []struct {
		name                    string
		initFuncs               []plugin.CoreRelayerChainInitFunc
		...
		{name: "2 evm chains with 3 nodes",
			initFuncs: []plugin.CoreRelayerChainInitFunc{
				plugin.InitEVM(testctx, factory, plugin.EVMFactoryConfig{
		...
		{name: "2 solana chain with 2 node",
			...
			initFuncs: []plugin.CoreRelayerChainInitFunc{
				plugin.InitSolana(testctx, factory, plugin.SolanaFactoryConfig{
		...
		{name: "2 starknet chain with 4 nodes",
			...
			initFuncs: []plugin.CoreRelayerChainInitFunc{
				plugin.InitStarknet(testctx, factory, plugin.StarkNetFactoryConfig{
...

		{
			...
			initFuncs: []plugin.CoreRelayerChainInitFunc{
				plugin.InitCosmos(testctx, factory, plugin.CosmosFactoryConfig{
		...
		{name: "all chains",
			...
			initFuncs: []plugin.CoreRelayerChainInitFunc{plugin.InitSolana(testctx, factory, plugin.SolanaFactoryConfig{
				Keystore:    keyStore.Solana(),
				TOMLConfigs: cfg.SolanaConfigs()}),
				plugin.InitEVM(testctx, factory, plugin.EVMFactoryConfig{
					ChainOpts: legacyevm.ChainOpts{
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			var cr *plugin.CoreRelayerChainInteroperators
			{
				var err error
				cr, err = plugin.NewCoreRelayerChainInteroperators(tt.initFuncs...)
			...
			for relayNetwork := range relay.SupportedNetworks {
				...
				interops := cr.List(plugin.FilterRelayersByType(relayNetwork))
				assert.Len(t, cr.List(plugin.FilterRelayersByType(relayNetwork)).Slice(), expectedChainCnt)
			...
			assert.ErrorIs(t, err, plugin.ErrNoSuchRelayer)
		})
	}
	...
	t.Run("bad init func", func(t *testing.T) {
		...
		badFunc := func() plugin.CoreRelayerChainInitFunc {
			return func(op *plugin.CoreRelayerChainInteroperators) error {
				return errBadFunc
			}
		}
		cr, err := plugin.NewCoreRelayerChainInteroperators(badFunc())
		assert.Nil(t, cr)
