...
import (
	...
	_ "github.com/goplugin/pluginv3.0/v2/core/services/pg"
...
func Test_ExternalInitiatorManager_Notify(t *testing.T) {
	...
	client.On("Do", mock.MatchedBy(func(r *http.Request) bool {
		...
		return r.Method == "POST" && r.URL.String() == eiWithURL.URL.String() && r.Header["Content-Type"][0] == "application/json" && r.Header["X-Plugin-Ea-Accesskey"][0] == "token" && r.Header["X-Plugin-Ea-Secret"][0] == "secret"
...
func Test_ExternalInitiatorManager_DeleteJob(t *testing.T) {
	...
	client.On("Do", mock.MatchedBy(func(r *http.Request) bool {
		...
		return r.Method == "DELETE" && r.URL.String() == expectedURL && r.Header["Content-Type"][0] == "application/json" && r.Header["X-Plugin-Ea-Accesskey"][0] == "token" && r.Header["X-Plugin-Ea-Secret"][0] == "secret"
	})).Once().Return(&http.Response{StatusCode: 200, Body: io.NopCloser(strings.NewReader(""))}, nil)
