...
import (
	...
	evmlogger "github.com/goplugin/pluginv3.0/v2/core/chains/evm/log"
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/batch_vrf_coordinator_v2"
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/vrf_consumer_v2"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/vrf_consumer_v2_upgradeable_example"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/vrf_coordinator_v2"
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/vrf_owner"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/vrf_single_consumer_example"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/vrfv2_proxy_admin"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/vrfv2_reverting_example"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/vrfv2_transparent_upgradeable_proxy"
...
type coordinatorV2UniverseCommon struct {
	...
	sergey       *bind.TransactOpts   // Owns all the PLI initially
...
func newVRFCoordinatorV2Universe(t *testing.T, key ethkey.KeyV2, numConsumers int) coordinatorV2Universe {
	...
	backend.Commit()
	// Deploy pli
	linkAddress, _, pliContract, err := pli_token_interface.DeployLinkToken(
		sergey, backend)
	require.NoError(t, err, "failed to deploy pli contract to simulated ethereum blockchain")

	// Deploy feed
	linkEthFeed, _, _, err :=
		mock_v3_aggregator_contract.DeployMockV3AggregatorContract(
			evil, backend, 18, vrftesthelpers.WeiPerUnitLink.BigInt()) // 0.01 eth per pli
	require.NoError(t, err)
	...
	coordinatorAddress, _, coordinatorContract, err :=
		vrf_coordinator_v2.DeployVRFCoordinatorV2(
			neil, backend, pliAddress, bhsAddress, pliEthFeed /* pliEth*/)
	...
	oldRootContractAddress, oldRootContract := deployOldCoordinator(
		t, pliAddress, bhsAddress, pliEthFeed, backend, neil)
	...
	for _, author := range vrfConsumers {
		// Deploy a VRF consumer. It has a starting balance of 500 PLI.
		consumerContractAddress, _, consumerContract, err2 :=
			vrf_consumer_v2.DeployVRFConsumerV2(
				author, backend, coordinatorAddress, pliAddress)
		require.NoError(t, err2, "failed to deploy VRFConsumer contract to simulated ethereum blockchain")
		_, err2 = pliContract.Transfer(sergey, consumerContractAddress, assets.Ether(500).ToInt()) // Actually, PLI
		require.NoError(t, err2, "failed to send PLI to VRFConsumer contract on simulated ethereum blockchain")
	...
	maliciousConsumerContractAddress, _, maliciousConsumerContract, err :=
		vrf_malicious_consumer_v2.DeployVRFMaliciousConsumerV2(
			evil, backend, coordinatorAddress, pliAddress)
	require.NoError(t, err, "failed to deploy VRFMaliciousConsumer contract to simulated ethereum blockchain")
	_, err = pliContract.Transfer(sergey, maliciousConsumerContractAddress, assets.Ether(1).ToInt()) // Actually, PLI
	require.NoError(t, err, "failed to send PLI to VRFMaliciousConsumer contract on simulated ethereum blockchain")
	backend.Commit()
	...
	require.NoError(t, err)
	initializeCalldata, err := upgradeableAbi.Pack("initialize", coordinatorAddress, pliAddress)
	hexified := hexutil.Encode(initializeCalldata)
	t.Log("initialize calldata:", hexified, "coordinator:", coordinatorAddress.String(), "link:", pliAddress)
	require.NoError(t, err)
	...
	_, err = pliContract.Transfer(sergey, proxyAddress, assets.Ether(500).ToInt()) // Actually, PLI
	require.NoError(t, err)
	backend.Commit()
	...
	lAddress, err := proxiedConsumer.PLITOKEN(nil)
	require.NoError(t, err)
	t.Log("link address in proxy to upgradeable consumer:", lAddress.String())
	require.Equal(t, pliAddress, lAddress)
	...
	revertingConsumerContractAddress, _, revertingConsumerContract, err := vrfv2_reverting_example.DeployVRFV2RevertingExample(
		reverter, backend, coordinatorAddress, pliAddress,
	)
	require.NoError(t, err, "failed to deploy VRFRevertingExample contract to simulated eth blockchain")
	_, err = pliContract.Transfer(sergey, revertingConsumerContractAddress, assets.Ether(500).ToInt()) // Actually, PLI
	require.NoError(t, err, "failed to send PLI to VRFRevertingExample contract on simulated eth blockchain")
	backend.Commit()
	...
	_, err = coordinatorContract.SetConfig(neil,
		...
		big.NewInt(1e16),                       // 0.01 eth per pli fallbackLinkPrice
	...
	return coordinatorV2Universe{
		coordinatorV2UniverseCommon: coordinatorV2UniverseCommon{
			...
			linkContract:                     pliContract,
			linkContractAddress:              pliAddress,
			linkEthFeedAddress:               pliEthFeed,
...
func deployOldCoordinator(
	...
	*vrf_coordinator_v2.VRFCoordinatorV2,
	...
	ctorArgs, err := evmutils.ABIEncode(`[{"type":"address"}, {"type":"address"}, {"type":"address"}]`, pliAddress, bhsAddress, pliEthFeed)
	require.NoError(t, err)
...
func subscribeAndAssertSubscriptionCreatedEvent(
	...
	// Create a subscription and fund with PLI.
...
func TestVRFV2Integration_SingleConsumer_NeedsTopUp(t *testing.T) {
	...
	testSingleConsumerNeedsTopUp(
		...
		uni.batchCoordinatorContractAddress,
		ptr(uni.vrfOwnerAddress),
		assets.Ether(1).ToInt(),   // initial funding of 1 PLI
		assets.Ether(100).ToInt(), // top up of 100 PLI
		vrfcommon.V2,
		false,
	)
}
...
func simulatedOverrides(t *testing.T, defaultGasPrice *assets.Wei, ks ...toml.KeySpecific) func(*plugin.Config, *plugin.Secrets) {
	return func(c *plugin.Config, s *plugin.Secrets) {
...
func TestExternalOwnerConsumerExample(t *testing.T) {
	...
	coordinatorAddress, _, coordinator, err :=
		vrf_coordinator_v2.DeployVRFCoordinatorV2(
			owner, backend, pliAddress, common.Address{}, common.Address{})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	backend.Commit()
	consumerAddress, _, consumer, err := vrf_external_sub_owner_example.DeployVRFExternalSubOwnerExample(owner, backend, coordinatorAddress, pliAddress)
	require.NoError(t, err)
	backend.Commit()
	_, err = pliContract.Transfer(owner, consumerAddress, assets.Ether(2).ToInt())
	require.NoError(t, err)
	backend.Commit()
	AssertLinkBalances(t, pliContract, []common.Address{owner.From, consumerAddress}, []*big.Int{assets.Ether(999_999_998).ToInt(), assets.Ether(2).ToInt()})
	...
	require.NoError(t, err)
	_, err = pliContract.TransferAndCall(owner, coordinatorAddress, big.NewInt(0), b)
	require.NoError(t, err)
...
func TestSimpleConsumerExample(t *testing.T) {
	...
	require.NoError(t, err)
	backend.Commit()
	consumerAddress, _, consumer, err := vrf_single_consumer_example.DeployVRFSingleConsumerExample(owner, backend, coordinatorAddress, pliAddress, 1, 1, 1, [32]byte{})
	require.NoError(t, err)
	backend.Commit()
	_, err = pliContract.Transfer(owner, consumerAddress, assets.Ether(2).ToInt())
	require.NoError(t, err)
	...
	require.NoError(t, err)
	backend.Commit()
	AssertLinkBalances(t, pliContract, []common.Address{owner.From, consumerAddress, coordinatorAddress}, []*big.Int{assets.Ether(999_999_998).ToInt(), assets.Ether(1).ToInt(), assets.Ether(1).ToInt()})
	...
	require.NoError(t, err)
	backend.Commit()
	AssertLinkBalances(t, pliContract, []common.Address{owner.From, consumerAddress, coordinatorAddress}, []*big.Int{assets.Ether(999_999_999).ToInt(), assets.Ether(0).ToInt(), assets.Ether(1).ToInt()})
	_, err = consumer.Unsubscribe(owner, owner.From)
	require.NoError(t, err)
	backend.Commit()
	AssertLinkBalances(t, pliContract, []common.Address{owner.From, consumerAddress, coordinatorAddress}, []*big.Int{assets.Ether(1_000_000_000).ToInt(), assets.Ether(0).ToInt(), assets.Ether(0).ToInt()})
}
...
func TestIntegrationVRFV2(t *testing.T) {
	...
	// Keep the prices low so we can operate with small pli balance subscriptions.
	...
	// We should be charged for the verification + our callbacks execution in pli.
	...
	linkCharged := pliWeiCharged.Sub(decimal.RequireFromString("1000000000000000")).Div(wei)
	gasPriceD := decimal.NewFromBigInt(gasPrice.ToInt(), 0)
	t.Logf("subscription charged %s with gas prices of %s gwei and %s ETH per PLI\n", pliCharged, gasPriceD.Div(gwei), vrftesthelpers.WeiPerUnitLink.Div(wei))
	...
	// wei/link * pli / wei/gas = wei / (wei/gas) = gas
	gasDiff := pliCharged.Sub(expected).Mul(vrftesthelpers.WeiPerUnitLink).Div(gasPriceD).Abs().IntPart()
	...
	_, err = uni.rootContract.OracleWithdraw(uni.nallory, uni.nallory.From, pliWeiCharged.Add(decimal.NewFromInt(1)).BigInt())
	require.Error(t, err)

	// Assert the oracle can withdraw its payment.
	_, err = uni.rootContract.OracleWithdraw(uni.nallory, uni.nallory.From, pliWeiCharged.BigInt())
	require.NoError(t, err)
	uni.backend.Commit()
	AssertLinkBalances(t, uni.linkContract, []common.Address{
		carolContractAddress,
...
func TestRequestCost(t *testing.T) {
	...
	t.Run("non-proxied consumer", func(tt *testing.T) {
		...
		_, err = carolContract.CreateSubscriptionAndFund(carol,
			big.NewInt(1000000000000000000)) // 0.1 PLI
	...
	t.Run("proxied consumer", func(tt *testing.T) {
		...
		// Create a subscription and fund with 5 PLI.
...
func AssertLinkBalance(t *testing.T, pliContract *link_token_interface.LinkToken, address common.Address, balance *big.Int) {
	b, err := pliContract.BalanceOf(nil, address)
	require.NoError(t, err)
...
func AssertLinkBalances(t *testing.T, pliContract *link_token_interface.LinkToken, addresses []common.Address, balances []*big.Int) {
	require.Equal(t, len(addresses), len(balances))
	for i, a := range addresses {
		AssertLinkBalance(t, pliContract, a, balances[i])
	}
}
