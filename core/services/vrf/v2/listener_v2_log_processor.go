...
// MaybeSubtractReservedLink figures out how much PLI is reserved for other VRF requests that
...
func (lsn *listenerV2) MaybeSubtractReservedLink(ctx context.Context, startBalance *big.Int, chainID *big.Int, subID *big.Int, vrfVersion vrfcommon.Version) (*big.Int, error) {
	...
	for _, tx := range txes {
		...
		if meta != nil && meta.MaxLink != nil {
			...
			if !success {
				return nil, fmt.Errorf("converting reserved PLI %s", *meta.MaxLink)
			}
...
func (lsn *listenerV2) processRequestsPerSubBatch(
	...
	if err != nil {
		lsn.l.Errorw("Couldn't get reserved PLI for subscription", "sub", reqs[0].req.SubID(), "err", err)
		return processed
	}
	...
	// Split the requests into native and PLI requests.
	var (
...
func (lsn *listenerV2) estimateFee(
	...
	// In the event we are using PLI we need to estimate the fee in juels
