...
func testMultipleConsumersNeedBHS(
	...
	for i := 0; i < nConsumers; i++ {
		...
		// Create a subscription and fund with 0 PLI.
		...
		topUpSubscription(t, consumer, consumerContract, uni.backend, big.NewInt(5e18 /* 5 PLI */), nativePayment)
...
func testSingleConsumerNeedsTopUp(
	...
	// Fulfillment will not be enqueued because subscriber doesn't have enough PLI.
	...
	// Top up subscription with enough PLI to see the job through.
...
func testSingleConsumerEIP150(
	...
	// Create a subscription and fund with 500 PLI.
...
func testSingleConsumerBigGasCallbackSandwich(
	...
	// Fulfillment will not be enqueued because subscriber doesn't have enough PLI for any of the requests.
	gomega.NewGomegaWithT(t).Consistently(func() bool {
		uni.backend.Commit()
		runs, err := app.PipelineORM().GetAllRuns(ctx)
		require.NoError(t, err)
...
func testSingleConsumerMultipleGasLanes(
	...
	// Top up subscription with enough PLI to see the job through. 100 PLI should do the trick.
