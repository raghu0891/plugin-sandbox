...
import (
	...
	v2 "github.com/goplugin/pluginv3.0/v2/core/services/vrf/v2"
...
func (d *Delegate) ServicesForSpec(ctx context.Context, jb job.Job) ([]job.ServiceCtx, error) {
	...
	for _, task := range pl.Tasks {
		if _, ok := task.(*pipeline.VRFTaskV2Plus); ok {
			...
			// Get the PLINATIVEFEED address with retries
			...
			err = retry.Do(func() error {
				linkNativeFeedAddress, err = coordinatorV2Plus.PLINATIVEFEED(nil)
				return err
			}, retry.Attempts(10), retry.Delay(500*time.Millisecond))
			if err != nil {
				return nil, errors.Wrap(err, "can't call PLINATIVEFEED")
			}
		...
		if _, ok := task.(*pipeline.VRFTaskV2); ok {
			...
			// Get the PLIETHFEED address with retries
			// This is needed because the RPC endpoint may be down so we need to
			// switch over to another one.
			var linkEthFeedAddress common.Address
			err = retry.Do(func() error {
				linkEthFeedAddress, err = coordinatorV2.PLIETHFEED(nil)
				return err
			}, retry.Attempts(10), retry.Delay(500*time.Millisecond))
			if err != nil {
				return nil, errors.Wrap(err, "PLIETHFEED")
			}
