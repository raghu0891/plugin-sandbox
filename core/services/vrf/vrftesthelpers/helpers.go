...
import (
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/solidity_vrf_request_id"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/solidity_vrf_request_id_v08"
...
func NewVRFCoordinatorUniverseWithV08Consumer(t *testing.T, key ethkey.KeyV2) CoordinatorUniverse {
	...
	_, err = cu.LinkContract.Transfer(cu.Sergey, consumerContractAddress, oneEth) // Actually, PLI
	require.NoError(t, err, "failed to send PLI to VRFConsumer contract on simulated ethereum blockchain")
	cu.Backend.Commit()
	return cu
}
...
func NewVRFCoordinatorUniverse(t *testing.T, keys ...ethkey.KeyV2) CoordinatorUniverse {
	...
	_, err = linkContract.Transfer(sergey, consumerContractAddress, oneEth) // Actually, PLI
	require.NoError(t, err, "failed to send PLI to VRFConsumer contract on simulated ethereum blockchain")
	backend.Commit()
