...
const decimals = 8
const fee = int64(100) // Amount paid by FA contract, in PLI-wei
...
func setupFluxAggregatorUniverse(t *testing.T, configOptions ...func(cfg *fluxAggregatorUniverseConfig)) fluxAggregatorUniverse {
	...
	f.backend.Commit() // Must commit contract to chain before we can fund with PLI
	...
	_, err = f.linkContract.Transfer(f.sergey, f.aggregatorContractAddress, oneEth) // Actually, PLI
	require.NoError(t, err, "failed to fund FluxAggregator contract with PLI")
...
func startApplication(
	...
	overrides func(c *plugin.Config, s *plugin.Secrets),
...
func awaitSubmission(t *testing.T, backend *backends.SimulatedBackend, submissionReceived chan *faw.FluxAggregatorSubmissionReceived) (
	...
	defer stopBlocks()
	select { // block until FluxAggregator contract acknowledges plugin message
	case log := <-submissionReceived:
		return log.Raw.BlockNumber, log.Submission.Int64()
	case <-time.After(20 * pollTimerPeriod):
		t.Fatal("plugin failed to submit answer to FluxAggregator contract")
...
func TestFluxMonitorAntiSpamLogic(t *testing.T) {
	...
	// Response by spammy plugin node, nallory
	//
	// The initial balance is the PLI balance of flux aggregator contract. We
...
"""
	...
	assertNoSubmission(t, submissionReceived, 5*pollTimerPeriod, "FA allowed plugin node to start a new round early")

	// Try to start a new round directly, should fail because of delay
	_, err = fa.aggregatorContract.RequestNewRound(fa.nallory)
	assert.Error(t, err, "FA allowed plugin node to start a new round early")
