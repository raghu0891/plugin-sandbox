...
// Taken from https://github.com/goplugin/plugin-testing-framework/blob/7ae47c88ecbb8a483ffe4d1e796704d1245f54d0/k8s/client/client.go
...
func (m *K8sClient) GetPodsWithConfigMap() ([]PodWithConfigMap, error) {
	...
	if len(pods.Items) == 0 {
		return nil, fmt.Errorf("no plugin node crib pods found, is your crib cluster deployed?")
	}
	...
	fmt.Printf("Found %d plugin node crib pods\n", len(podsWithConfigMaps))
