...
import (
	...
	link "github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/link_token_interface"
...
const (
	defaultPluginNodeLogin    = "notreal@fakeemail.ch"
	defaultPluginNodePassword = "fj293fbBnlQ!f9vNs~#"
...
func (h *baseHandler) launchPluginNode(ctx context.Context, port int, containerName string, extraTOML string, force bool) (string, func(bool), error) {
	...
	if _, _, err = dockerClient.ImageInspectWithRaw(ctx, h.cfg.PluginDockerImage); err != nil {
		log.Println("Pulling node docker image...")
		if out, err = dockerClient.ImagePull(ctx, h.cfg.PluginDockerImage, types.ImagePullOptions{}); err != nil {
	...
	// Create temporary file with plugin node login creds
	...
	nodeContainerResp, err := dockerClient.ContainerCreate(ctx, &container.Config{
		Image: h.cfg.PluginDockerImage,
		Cmd:   []string{"-s", "/run/secrets/01-secret.toml", "-c", "/run/secrets/01-config.toml", "local", "n", "-a", "/run/secrets/plugin-node-api"},
		Env: []string{
			"CL_CONFIG=" + extraTOML,
			"CL_PASSWORD_KEYSTORE=" + defaultPluginNodePassword,
		...
		Mounts: []mount.Mount{
			{
				Type:   mount.TypeBind,
				Source: apiFile,
				Target: "/run/secrets/plugin-node-api",
			},
			{
				Type:   mount.TypeBind,
				Source: passwordFile,
				Target: "/run/secrets/plugin-node-password",
			},
			{
				Type:   mount.TypeBind,
				Source: tomlFile,
...
// getNodeAddress returns plugin node's wallet address
...
// getNodeOCR2Config returns plugin node's OCR2 bundle key ID
...
// getP2PKeyID returns plugin node's P2P key ID
...
func createCredsFiles() (string, string, func(), error) {
	// Create temporary file with plugin node login creds
	apiFile, err := os.CreateTemp("", "plugin-node-api")
	if err != nil {
		return "", "", nil, fmt.Errorf("failed to create api file: %w", err)
	}
	_, _ = apiFile.WriteString(defaultPluginNodeLogin)
	_, _ = apiFile.WriteString("\n")
	_, _ = apiFile.WriteString(defaultPluginNodePassword)

	// Create temporary file with plugin node password
	passwordFile, err := os.CreateTemp("", "plugin-node-password")
	if err != nil {
		return "", "", nil, fmt.Errorf("failed to create password file: %w", err)
	}
	_, _ = passwordFile.WriteString(defaultPluginNodePassword)
...
func createTomlFile(tomlString string) (string, func(), error) {
	// Create temporary file with plugin node TOML config
	tomlFile, err := os.CreateTemp("", "plugin-toml-config")
	if err != nil {
		return "", nil, fmt.Errorf("failed to create toml file: %w", err)
	}
