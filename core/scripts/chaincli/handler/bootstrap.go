...
func (h *baseHandler) StartBootstrapNode(ctx context.Context, addr string, uiPort, p2pv2Port int, force bool) string {
	...
	urlRaw, _, err := h.launchPluginNode(
		ctx,
		uiPort,
		containerName,
	...
	if err != nil {
		lggr.Fatal("Failed to launch plugin node, ", err)
	}

	cl, err := authenticate(ctx, urlRaw, defaultPluginNodeLogin, defaultPluginNodePassword, lggr)
	if err != nil {
	...
	lggr.Info("Bootstrap job has been successfully created in the Plugin node with address ", urlRaw, ", tcp: ", tcpAddr)

	return tcpAddr
}

// createBootstrapJob creates a bootstrap job in the plugin node by the given address
func (h *baseHandler) createBootstrapJob(ctx context.Context, client cmd.HTTPClient, contractAddr string) error {
	...
	log.Println("Bootstrap job has been successfully created in the Plugin node")
	return nil
}
