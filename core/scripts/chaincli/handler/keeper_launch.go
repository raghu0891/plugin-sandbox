...
// LaunchAndTest launches keeper registry, plugin nodes, upkeeps and start performing.
// 1. launch plugin node using docker image
...
func (k *Keeper) LaunchAndTest(ctx context.Context, withdraw, printLogs, force, bootstrap bool) {
	...
	// Run plugin nodes and create jobs
	...
	for i := 0; i < k.cfg.KeepersCount; i++ {
		...
		go func(i int) {
			...
			// Run plugin node
			var err error
			if startedNodes[i].url, startedNodes[i].cleanup, err = k.launchPluginNode(ctx, 6688+i, fmt.Sprintf("keeper-%d", i), extraTOML, force); err != nil {
	...
	for i, startedNode := range startedNodes {
		...
		var err error
		cl, err = authenticate(ctx, startedNode.url, defaultPluginNodeLogin, defaultPluginNodePassword, lggr)
		if err != nil {
		...
		if !ok {
			log.Printf("failed to parse FUND_PLUGIN_NODE: %s", k.cfg.FundNodeAmount)
			continue
		}
		if fundAmt.Cmp(big.NewInt(0)) != 0 {
			if err = k.sendEth(ctx, nodeAddr, fundAmt); err != nil {
				log.Println("Failed to fund plugin node: ", err)
				continue
			}
		}
...
// createKeeperJob creates a keeper job in the plugin node by the given address
func (k *Keeper) createKeeperJob(ctx context.Context, client cmd.HTTPClient, registryAddr, nodeAddr string) error {
	...
	log.Println("Keeper job has been successfully created in the Plugin node with address: ", nodeAddr)

	return nil
}

// createLegacyKeeperJob creates a legacy keeper job in the plugin node by the given address
...
// createOCR2KeeperJob creates an ocr2keeper job in the plugin node by the given address
...
func (k *Keeper) addKeyToKeeper(ctx context.Context, client cmd.HTTPClient, privKeyHex string) (string, error) {
	...
	keyJSON, err := ethkey.FromPrivateKey(privkey).ToEncryptedJSON(defaultPluginNodePassword, utils.FastScryptParams)
	if err != nil {
	...
	query.Set("oldpassword", defaultPluginNodePassword)
