...
4. The private key of an account funded with PLI, and the chain's native token (to pay transaction fees)
...
5. The PLI address, PLI-ETH feed address, fast gas feed address for your chain
...
The example .env in this repo is for the Polygon Mumbai testnet. You can use [this faucet](https://faucets.chain.link/mumbai) to send testnet PLI
...
Build a local copy of the plugin docker image by running this command in the root directory of the plugin repo:

```bash
docker build -t plugin:local -f ./core/plugin.Dockerfile .
```
...
Other options include:
- `--ui-port`: default `5688`, the Plugin node UI listen port
- `--p2pv2-port`: default `8000`, the Plugin node P2P listen port
...
```
In the output of this command, you will see the http address of the nodes, e.g. `http://localhost:6688`. This is the Plugin Operator GUI. You can use the default username `notreal@fakeemail.ch` and password `fj293fbBnlQ!f9vNs~#` to log in.
