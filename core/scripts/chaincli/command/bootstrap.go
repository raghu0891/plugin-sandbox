...
// BootstrapNodeCmd launches a plugin node with a bootstrap job
var BootstrapNodeCmd = &cobra.Command{
	Use:   "bootstrap [ui-port] [p2pv2-port]",
	Short: "Setup a bootstrap node.",
	Long:  `This commands launches a plugin node inside the docker container and sets up the bootstrap job`,
...
func init() {
	...
	BootstrapNodeCmd.Flags().Int("ui-port", 5688, "Plugin node UI listen port")
	BootstrapNodeCmd.Flags().Int("p2pv2-port", 8000, "Plugin node P2P listen port")
}
