...
database="plugin_development_test"
...
SELECT 'CREATE DATABASE plugin_test_pristine WITH OWNER $username;' 
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'plugin_test_pristine')\gexec
EOF
