...
import (
	...
	"github.com/goplugin/pluginv3.0/core/scripts/common/vrf/constants"
	"github.com/goplugin/pluginv3.0/core/scripts/common/vrf/jobs"
	"github.com/goplugin/pluginv3.0/core/scripts/common/vrf/model"
	"github.com/goplugin/pluginv3.0/core/scripts/common/vrf/util"
...
func SmokeTestVRF(e helpers.Environment) {
	...
	flatFeeLinkDiscountPPM := smokeCmd.Int64("flat-fee-link-discount-ppm", 100, "fulfillment flat fee discount for PLI payment denominated in native ppm")
	nativePremiumPercentage := smokeCmd.Int64("native-premium-percentage", 1, "premium percentage for native payment")
	linkPremiumPercentage := smokeCmd.Int64("link-premium-percentage", 1, "premium percentage for PLI payment")
	...
	fmt.Println(
		...
		"\nPLI/Native Feed contract address:", *linkNativeAddress,
...
func DeployUniverseViaCLI(e helpers.Environment) {
	...
	flatFeeLinkDiscountPPM := deployCmd.Int64("flat-fee-link-discount-ppm", 100, "fulfillment flat fee discount for PLI payment denominated in native ppm")
	nativePremiumPercentage := deployCmd.Int64("native-premium-percentage", 1, "premium percentage for native payment")
	linkPremiumPercentage := deployCmd.Int64("link-premium-percentage", 1, "premium percentage for PLI payment")
...
func VRFV2PlusDeployUniverse(e helpers.Environment,
	...
	fmt.Println(
		...
		"\nPLI/Native Feed contract address:", contractAddresses.LinkEthAddress,
...
func DeployWrapperUniverse(e helpers.Environment) {
	...
	subFundingLink := cmd.String("sub-funding-link", "10000000000000000000", "amount in PLI to fund the subscription with")
	subFundingNative := cmd.String("sub-funding-native", "10000000000000000000", "amount in native to fund the subscription with")
	consumerFundingLink := cmd.String("consumer-funding-link", "10000000000000000000", "amount in PLI to fund the consumer with")
	...
	fmt.Printf("Send PLI request example: go run . wrapper-consumer-request --consumer-address=%s --cb-gas-limit=1000000 --native-payment=false\n", consumer.String())
}
