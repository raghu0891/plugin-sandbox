...
func TestTxm_NewDynamicFeeTx(t *testing.T) {
	...
	t.Run("verifies gas tip and fees", func(t *testing.T) {
		tests := []struct {
			...
			tipcap      *assets.Wei
			feecap      *assets.Wei
			setCfg      func(*plugin.Config, *plugin.Secrets)
			expectError string
		}{
			...
			{"fee cap exceeds max allowed", assets.GWei(5), assets.GWei(5), func(c *plugin.Config, s *plugin.Secrets) {
			...
			{"ignores global min gas price", assets.GWei(5), assets.GWei(5), func(c *plugin.Config, s *plugin.Secrets) {
				c.EVM[0].GasEstimator.PriceMin = assets.GWei(6)
			}, ""},
			{"tip cap below min allowed", assets.GWei(5), assets.GWei(5), func(c *plugin.Config, s *plugin.Secrets) {
