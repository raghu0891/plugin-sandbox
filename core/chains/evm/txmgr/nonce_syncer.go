...
//
// The problem with doing this is that now Plugin does not have any
...
func (s nonceSyncerImpl) fastForwardNonceIfNecessary(ctx context.Context, address common.Address, localNonce types.Nonce) (types.Nonce, error) {
	...
	s.logger.Warnw(fmt.Sprintf("address %s has been used before, either by an external wallet or a different Plugin node. "+
		...
		"Please note that using the plugin keys with an external wallet is NOT SUPPORTED and can lead to missed or stuck transactions. ",
