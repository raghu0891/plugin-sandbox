...
func (f InstanceAppFactory) NewApplication(context.Context, plugin.GeneralConfig, logger.Logger, *sqlx.DB) (plugin.Application, error) {
	return f.App, nil
}
...
func (s seededAppFactory) NewApplication(context.Context, plugin.GeneralConfig, logger.Logger, *sqlx.DB) (plugin.Application, error) {
...
func (r BlockedRunner) Run(context.Context, plugin.Application) error {
	<-r.Done
	return nil
}
...
func (r EmptyRunner) Run(context.Context, plugin.Application) error {
	return nil
}
...
func MustRandomUser(t testing.TB) sessions.User {
	email := fmt.Sprintf("user-%v@plugin.test", NewRandomPositiveInt64())
