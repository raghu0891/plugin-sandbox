...
// NewTestGeneralConfig returns a new plugin.GeneralConfig with default test overrides and one chain with evmclient.NullClientChainID.
func NewTestGeneralConfig(t testing.TB) plugin.GeneralConfig { return NewGeneralConfig(t, nil) }

// NewGeneralConfig returns a new plugin.GeneralConfig with overrides.
// The default test overrides are applied before overrideFn, and include one chain with evmclient.NullClientChainID.
func NewGeneralConfig(t testing.TB, overrideFn func(*plugin.Config, *plugin.Secrets)) plugin.GeneralConfig {
	tempDir := t.TempDir()
	g, err := plugin.GeneralConfigOpts{
		OverrideFn: func(c *plugin.Config, s *plugin.Secrets) {
			overrides(c, s)
			c.RootDir = &tempDir
...
func overrides(c *plugin.Config, s *plugin.Secrets) {
...
// NewGeneralConfigSimulated returns a new plugin.GeneralConfig with overrides, including the simulated EVM chain.
...
func NewGeneralConfigSimulated(t testing.TB, overrideFn func(*plugin.Config, *plugin.Secrets)) plugin.GeneralConfig {
	return NewGeneralConfig(t, func(c *plugin.Config, s *plugin.Secrets) {
		simulated(c, s)
		if fn := overrideFn; fn != nil {
			fn(c, s)
		}
...
func simulated(c *plugin.Config, s *plugin.Secrets) {
