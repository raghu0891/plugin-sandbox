...
import (
	...
	"github.com/goplugin/pluginv3.0/v2/core/shutdown"
...
// RunNode starts the Plugin core.
...
func checkFilePermissions(lggr logger.Logger, rootDir string) error {
	...
	for _, fileName := range protectedFiles {
		...
		owned, err := utils.IsFileOwnedByPlugin(fileInfo)
		if err != nil {
			lggr.Warn(err)
			continue
		}
		if !owned {
			lggr.Warnf("The file %v is not owned by the user running plugin. This will be made mandatory in the future.", path)
		}
	}
	return nil
}
...
var errDBURLMissing = errors.New("You must set CL_DATABASE_URL env variable or provide a secrets TOML with Database.URL set. HINT: If you are running this to set up your local test database, try CL_DATABASE_URL=postgresql://postgres@localhost:5432/plugin_test?sslmode=disable")
