...
import (
	...
	"github.com/goplugin/pluginv3.0/v2/core/services/versioning"
	...
	"github.com/goplugin/pluginv3.0/v2/core/store/migrate"
	"github.com/goplugin/pluginv3.0/v2/core/utils"
	clhttp "github.com/goplugin/pluginv3.0/v2/core/utils/http"
...
type Shell struct {
	Renderer
	Config                         plugin.GeneralConfig // initialized in Before
...
type AppFactory interface {
	NewApplication(ctx context.Context, cfg plugin.GeneralConfig, appLggr logger.Logger, db *sqlx.DB) (plugin.Application, error)
}

// PluginAppFactory is used to create a new Application.
type PluginAppFactory struct{}
...
func (n PluginAppFactory) NewApplication(ctx context.Context, cfg plugin.GeneralConfig, appLggr logger.Logger, db *sqlx.DB) (app plugin.Application, err error) {
	...
	if cfg.SolanaEnabled() {
		...
		initOps = append(initOps, plugin.InitSolana(ctx, relayerFactory, solanaCfg))
	}
	...
	return plugin.NewApplication(plugin.ApplicationOpts{
		...
		SecretGenerator:            plugin.FilePersistedSecretGenerator{},
		LoopRegistry:               loopRegistry,
		GRPCOpts:                   grpcOpts,
		MercuryPool:                mercuryPool,
		CapabilitiesRegistry:       capabilitiesRegistry,
...
type Runner interface {
	Run(context.Context, plugin.Application) error
}

// PluginRunner is used to run the node application.
type PluginRunner struct{}

// Run sets the log level based on config and starts the web router to listen
// for input and return data.
func (n PluginRunner) Run(ctx context.Context, app plugin.Application) error {
...
// HTTPClient encapsulates all methods used to interact with a plugin node API.
...
func NewUserCache(subdir string, lggr func() logger.Logger) (*UserCache, error) {
	...
	return &UserCache{dir: filepath.Join(cd, "plugin", subdir), lggr: lggr}, nil
}
