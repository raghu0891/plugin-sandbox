...
import (
	...
	cmdMocks "github.com/goplugin/pluginv3.0/v2/core/cmd/mocks"
...
func genTestEVMRelayers(t *testing.T, opts legacyevm.ChainRelayExtenderConfig, ks evmrelayer.CSAETHKeystore) *plugin.CoreRelayerChainInteroperators {
	f := plugin.RelayerFactory{
		Logger:       opts.Logger,
		LoopRegistry: plugins.NewLoopRegistry(opts.Logger, opts.AppConfig.Tracing()),
	}

	relayers, err := plugin.NewCoreRelayerChainInteroperators(plugin.InitEVM(testutils.Context(t), f, plugin.EVMFactoryConfig{
...
func TestShell_RebroadcastTransactions_Txm(t *testing.T) {
	...
	config, sqlxDB := heavyweight.FullTestDBV2(t, func(c *plugin.Config, s *plugin.Secrets) {
	...
	mockRelayerChainInteroperators := &pluginmocks.FakeRelayerChainInteroperators{EVMChains: legacy}
...
func TestShell_CleanupChainTables(t *testing.T) {
	...
	config, _ := heavyweight.FullTestDBV2(t, func(c *plugin.Config, s *plugin.Secrets) { c.Database.Dialect = dialects.Postgres })
	client := cmd.Shell{
		Config: config,
	...
	// heavyweight creates test db named plugin_test_uid, while usual naming is plugin_test
	// CleanupChainTables handles test db name with plugin_test, but because of heavyweight test db naming we have to set danger flag
