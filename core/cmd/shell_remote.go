...
func initRemoteConfigSubCmds(s *Shell) []cli.Command {
	return []cli.Command{
		...
		{
			Name:  "validate",
			Usage: "DEPRECATED. Use `plugin node validate`",
			Before: func(c *cli.Context) error {
				return s.errorOut(fmt.Errorf("Deprecated, use `plugin node validate`"))
			},
			Hidden: true,
		},
	}
...
// posts it to Plugin to change the password.
...
func (s *Shell) parseResponse(resp *http.Response) ([]byte, error) {
	...
	if errors.Is(err, errUnauthorized) {
		return nil, s.errorOut(multierr.Append(err, fmt.Errorf("your credentials may be missing, invalid or you may need to login first using the CLI via 'plugin admin login'")))
	}
