...
func SetupLocalLoadTestEnv(globalConfig tc.GlobalTestConfig, functionsConfig types.FunctionsTestConfig) (*FunctionsTest, error) {
	...
	lt, err := cl.LoadPLIToken(*cfg.Common.PLITokenAddr)
	if err != nil {
		return nil, err
	}
	...
	if cfg.Common.SubscriptionID == nil {
		...
		encodedSubId, err := pluginutils.ABIEncode(`[{"type":"uint64"}]`, subID)
		if err != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("failed to transferAndCall router, PLI funding: %w", err)
		}
