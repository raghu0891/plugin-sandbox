...
func sendSlackNotification(header string, l zerolog.Logger, config *tc.TestConfig, namespace string, numberOfNodes,
	...
	headerText := ":plugin-keepers: Automation Load Test " + header
	...
	if *config.Pyroscope.Enabled {
		...
		formattedPyroscopeUrl := fmt.Sprintf("%s/?query=plugin-node.cpu{Environment=\"%s\"}&from=%s&to=%s", pyroscopeServer, pyroscopeEnvironment, startingTime, endingTime)
