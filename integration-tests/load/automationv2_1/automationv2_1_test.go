...
import (
	...
	"github.com/goplugin/plugin-testing-framework/lib/k8s/pkg/helm/wiremock"
...
%s`
	...
	for i := 0; i < numberOfUpkeeps+1; i++ { // +1 for the OCR boot node
		var overrideFn = func(_ interface{}, target interface{}) {
			ctfconfig.MustConfigOverridePluginVersion(loadedTestConfig.GetPluginImageConfig(), target)
	...
	a := automationv2.NewAutomationTestK8s(l, chainClient, pluginNodes, &loadedTestConfig)
	...
	err = actions.FundPluginNodesFromRootAddress(l, a.ChainClient, contracts.PluginK8sClientToPluginNodeWithKeysAndAddress(pluginNodes[1:]), big.NewFloat(*loadedTestConfig.Common.PluginNodeFunding))
...
Test Duration: %s`
	...
	t.Cleanup(func() {
		if err = actions.TeardownRemoteSuite(t, chainClient, testEnvironment.Cfg.Namespace, pluginNodes, nil, &loadedTestConfig); err != nil {
