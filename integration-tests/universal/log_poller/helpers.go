...
import (
	...
	ac "github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/automation_compatible_utils"
...
func SetupLogPollerTestDocker(
	...
	logScannerSettings test_env.PluginNodeLogScannerSettings,
) (
	*seth.Client,
	[]*client.PluginClient,
	contracts.LinkToken,
	...
	*blockchain.EVMNetwork,
	...
	pluginNodeFunding := 0.5
	l.Debug().Msgf("Funding amount: %f", pluginNodeFunding)
	clNodesCount := 5
	...
	env, err = test_env.NewCLTestEnvBuilder().
		...
		WithPluginNodeLogScanner(logScannerSettings).
		WithStandardCleanup().
		Build()
	...
	err = actions.FundPluginNodesFromRootAddress(l, chainClient, contracts.PluginClientToPluginNodeWithKeysAndAddress(env.ClCluster.NodeAPIs()), big.NewFloat(pluginNodeFunding))
	...
	require.NoError(t, err, "Error getting PLI balance")

	l.Info().Str("Balance", big.NewInt(0).Div(linkBalance, big.NewInt(1e18)).String()).Msg("PLI balance")
	...
	if linkBalance.Cmp(minLinkBalance) < 0 {
		require.FailNowf(t, "Not enough PLI", "Not enough PLI to run the test. Need at least %s. but has only %s", big.NewInt(0).Div(minLinkBalance, big.NewInt(1e18)).String(), big.NewInt(0).Div(linkBalance, big.NewInt(1e18)).String())
	}
