...
func CreateKeeperJobsWithKeyIndex(
	...
	require.NoError(t, err, "Reading ETH Keys from Plugin Client shouldn't fail")
	nodeAddresses, err := PluginNodeAddressesAtIndex(pluginNodes, keyIndex)
	...
	for _, pluginNode := range pluginNodes {
		pluginNodeAddress, err := pluginNode.EthAddresses()
		require.NoError(t, err, "Error retrieving plugin node address")
		_, err = pluginNode.MustCreateJob(&client.KeeperJobSpec{
			Name:                     fmt.Sprintf("keeper-test-%s", keeperRegistry.Address()),
			ContractAddress:          keeperRegistry.Address(),
			FromAddress:              pluginNodeAddress[keyIndex],
...
func DeleteKeeperJobsWithId(t *testing.T, pluginNodes []*client.PluginK8sClient, id int) {
	for _, pluginNode := range pluginNodes {
		err := pluginNode.MustDeleteJob(strconv.Itoa(id))
...
func DeployKeeperContracts(
	...
	ef, err := contracts.DeployMockPLIETHFeed(client, big.NewInt(2e18))
	...
	// Fund the registry with 1 PLI * amount of KeeperConsumerPerformance contracts
...
func DeployPerformanceKeeperContracts(
	...
	ef, err := contracts.DeployMockPLIETHFeed(chainClient, big.NewInt(2e18))
	require.NoError(t, err, "Deploying mock ETH-Link feed shouldn't fail")
...
func RegisterUpkeepContractsWithCheckData(t *testing.T, client *seth.Client, linkToken contracts.LinkToken, fundsForEachUpkeep *big.Int, upkeepGasLimit uint32, registry contracts.KeeperRegistry, registrar contracts.KeeperRegistrar, numberOfContracts int, upkeepAddresses []string, checkData [][]byte, isLogTrigger bool, isMercury bool, isBillingTokenNative bool, wethToken contracts.WETHToken) []*big.Int {
	...
	var registerUpkeepFn = func(resultCh chan upkeepRegistrationResult, errorCh chan error, executorNum int, config upkeepConfig) {
		...
		if isBillingTokenNative {
			...
			if err != nil {
				errorCh <- errors.Wrapf(err, "[id: %s]Failed to get PLI balance of %s", id, client.Addresses[keyNum].Hex())
				return
			}

			// not strictly necessary, but helps us to avoid an errorless revert if there is not enough PLI
			if balance.Cmp(fundsForEachUpkeep) < 0 {
				errorCh <- fmt.Errorf("[id: %s] Not enough PLI balance for %s. Has: %s. Needs: %s", id, client.Addresses[keyNum].Hex(), balance.String(), fundsForEachUpkeep.String())
				return
			}
