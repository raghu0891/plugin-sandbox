...
import (
	...
	vrfv2plusconfig "github.com/goplugin/pluginv3.0/integration-tests/testconfig/vrfv2plus"
...
func CreateVRFV2PlusJob(
	...
	job, err := pluginNode.MustCreateJob(&jobSpec)
	if err != nil {
...
func SetupVRFV2_5Environment(
	...
	mockNativePLIFeed contracts.VRFMockETHPLIFeed,
...
func SetupVRFV2PlusWrapperForNewEnv(
	...
	wrapperContracts, err := DeployVRFV2PlusDirectFundingContracts(
		...
		vrfContracts.MockETHPLIFeed.Address(),
		vrfContracts.CoordinatorV2Plus,
...
func SetupVRFV2PlusForNewEnv(
	...
	mockETHLinkFeed, err := contracts.DeployVRFMockETHPLIFeed(sethClient, big.NewInt(*envConfig.TestConfig.VRFv2Plus.General.LinkNativeFeedResponse))
	if err != nil {
		return nil, nil, nil, nil, nil, fmt.Errorf("%s, err: %w", "error deploying mock ETH/PLI feed", err)
	}
	linkToken, err := contracts.DeployLinkTokenContract(l, sethClient)
	if err != nil {
		return nil, nil, nil, nil, nil, fmt.Errorf("%s, err: %w", "error deploying PLI contract", err)
	}
