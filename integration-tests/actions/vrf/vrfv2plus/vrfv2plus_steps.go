...
import (
	...
	testconfig "github.com/goplugin/pluginv3.0/integration-tests/testconfig/vrfv2"
	...
	pluginutils "github.com/goplugin/pluginv3.0/v2/core/chains/evm/utils"
...
func CreateVRFV2PlusJob(
	...
	job, err := pluginNode.MustCreateJob(&client.VRFV2PlusJobSpec{
...
func FundVRFCoordinatorV2_5Subscription(
	...
	encodedSubId, err := pluginutils.ABIEncode(`[{"type":"uint256"}]`, subscriptionID)
	if err != nil {
...
func SetupVRFV2_5Environment(
	...
	l.Info().Str("Coordinator", vrfContracts.CoordinatorV2Plus.Address()).Msg("Setting Link and ETH/PLI feed")
	err = vrfContracts.CoordinatorV2Plus.SetPLIAndPLINativeFeed(linkToken.Address(), mockNativePLIFeed.Address())
	if err != nil {
	...
	vrfTXKeyAddressStrings, _, err := vrfcommon.CreateFundAndGetSendingKeys(
		...
		*vrfv2PlusTestConfig.GetCommonConfig().PluginNodeFunding,
...
func ReturnFundsForFulfilledRequests(client blockchain.EVMClient, coordinator contracts.VRFCoordinatorV2_5, l zerolog.Logger) error {
	linkTotalBalance, err := coordinator.GetLinkTotalBalance(context.Background())
	if err != nil {
		return fmt.Errorf("Error getting PLI total balance, err: %w", err)
	}
	defaultWallet := client.GetDefaultWallet().Address()
	l.Info().
		Str("PLI amount", linkTotalBalance.String()).
		Str("Returning to", defaultWallet).
		Msg("Returning PLI for fulfilled requests")
	err = coordinator.Withdraw(
		common.HexToAddress(defaultWallet),
	)
	if err != nil {
		return fmt.Errorf("Error withdrawing PLI from coordinator to default wallet, err: %w", err)
	}
