...
func CreateFundAndGetSendingKeys(
	...
	client *seth.Client,
	node *VRFNode,
	pluginNodeFunding float64,
	numberOfTxKeysToCreate int,
	chainID *big.Int,
) ([]string, []common.Address, error) {
	newNativeTokenKeyAddresses, err := CreateAndFundSendingKeys(l, client, node, pluginNodeFunding, numberOfTxKeysToCreate, chainID)
	if err != nil {
		return nil, nil, err
	}
...
func CreateAndFundSendingKeys(
	...
	for i := 0; i < numberOfNativeTokenAddressesToCreate; i++ {
		...
		_, err = actions.SendFunds(l, client, actions.FundsToSendPayload{
			...
			Amount:     conversions.EtherToWei(big.NewFloat(pluginNodeFunding)),
...
func BuildNewCLEnvForVRF(l zerolog.Logger, t *testing.T, envConfig VRFEnvConfig, newEnvConfig NewEnvConfig, network ctf_test_env.EthereumNetwork) (*test_env.CLClusterTestEnv, *seth.Client, error) {
	env, err := test_env.NewCLTestEnvBuilder().
		...
		WithPluginNodeLogScanner(newEnvConfig.PluginNodeLogScannerSettings).
	...
	err = actions.FundPluginNodesFromRootAddress(l, sethClient, contracts.PluginClientToPluginNodeWithKeysAndAddress(env.ClCluster.NodeAPIs()), big.NewFloat(*envConfig.TestConfig.Common.PluginNodeFunding))
	if err != nil {
