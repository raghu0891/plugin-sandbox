...
// ReturnFundsFromNodes returns funds from the given plugin nodes to the default network wallet. It will use a variety
...
func ReturnFundsFromNodes(log zerolog.Logger, client *seth.Client, pluginNodes []contracts.PluginNodeWithKeysAndAddress) error {
...
func ReturnFundsFromKeyExporterNodes(log zerolog.Logger, client *seth.Client, pluginNodes []contracts.PluginKeyExporter) error {
	if client == nil {
		return fmt.Errorf("seth client is nil, unable to return funds from plugin nodes")
	}
	...
	for _, pluginNode := range pluginNodes {
		fundedKeys, err := pluginNode.ExportEVMKeysForChain(fmt.Sprint(client.ChainID))
		if err != nil {
			return err
		}
		for _, key := range fundedKeys {
			...
			decryptedKey, err := keystore.DecryptKey(keyToDecrypt, clClient.PluginKeyPassword)
			if err != nil {
				return err
			}
			...
			if err != nil {
				log.Error().Err(err).Msg("Failed to return funds from Plugin node to default network wallet")
	...
	if len(failedReturns) > 0 {
		return fmt.Errorf("failed to return funds from Plugin nodes to default network wallet for addresses: %v", failedReturns)
	}

	log.Info().Msg("Successfully returned funds from all Plugin nodes to default network wallets")
	...
	if err != nil {
		...
		if err != nil {
			log.Error().
				Err(err).
				Str("Address", fromAddress.String()).
				Msg("Failed to return funds from Plugin node to default network wallet")
