...
// PluginNodeAddressesAtIndex will return all the on-chain wallet addresses for a set of Plugin nodes
func PluginNodeAddressesAtIndex(nodes []*client.PluginK8sClient, keyIndex int) ([]common.Address, error) {
	addresses := make([]common.Address, 0)
	for _, node := range nodes {
...
func DeleteAllJobs(pluginNodes []*client.PluginK8sClient) error {
	for _, node := range pluginNodes {
		if node == nil {
			return fmt.Errorf("found a nil plugin node in the list of plugin nodes while tearing down: %v", pluginNodes)
		}
		jobs, _, err := node.ReadJobs()
		if err != nil {
			return fmt.Errorf("error reading jobs from plugin node, err: %w", err)
		}
		for _, maps := range jobs.Data {
			...
			if err != nil {
				return fmt.Errorf("error deleting job from plugin node, err: %w", err)
			}
		}
	}
	return nil
