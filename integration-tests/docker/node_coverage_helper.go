...
type NodeCoverageHelper struct {
	Nodes            []tc.Container
	GoCoverSrcDir    string   // Path to the source directory on the plugin image with go coverage data
	...
	PluginDir     string   // Path to the root plugin directory
}

func NewNodeCoverageHelper(ctx context.Context, nodes []tc.Container, pluginDir, coverageDir string) (*NodeCoverageHelper, error) {
...
func (c *NodeCoverageHelper) mergeCoverage() error {
	...
	// Merge the coverage data from all plugin nodes
