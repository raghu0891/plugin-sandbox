...
func (c *ClCluster) NodeAPIs() []*client.PluginClient {
	clients := make([]*client.PluginClient, 0)
...
// Return all the on-chain wallet addresses for a set of Plugin nodes
func (c *ClCluster) NodeAddresses() ([]common.Address, error) {
	addresses := make([]common.Address, 0)
	for _, n := range c.Nodes {
		primaryAddress, err := n.PluginNodeAddress()
		if err != nil {
			return nil, err
		}
