...
import (
	...
	envclient "github.com/goplugin/plugin-testing-framework/lib/k8s/client"
...
var (
	pluginResources = map[string]interface{}{
...
func SetupAutomationBenchmarkEnv(t *testing.T, keeperTestConfig types.AutomationBenchmarkTestConfig) (*environment.Environment, blockchain.EVMNetwork) {
	...
	if testNetwork.Simulated {
		...
		// Need to setup geth node before setting up plugin nodes
	...
	for i := 0; i < numberOfNodes; i++ {
		...
		var overrideFn = func(_ interface{}, target interface{}) {
			ctfconfig.MustConfigOverridePluginVersion(keeperTestConfig.GetPluginImageConfig(), target)
