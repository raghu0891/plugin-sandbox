...
import (
	...
	env_client "github.com/goplugin/plugin-testing-framework/k8s/client"
	...
	"github.com/goplugin/plugin-testing-framework/k8s/pkg/helm/reorg"
...
LimitDefault = 5_000_000`
	...
	performancePluginResources = map[string]interface{}{
	...
	soakPluginResources = map[string]interface{}{
		"resources": map[string]interface{}{
			"requests": map[string]interface{}{
				"cpu":    "350m",
				"memory": "1Gi",
...
func TestAutomationBenchmark(t *testing.T) {
	...
	keeperBenchmarkTest := testsetups.NewKeeperBenchmarkTest(t,
		testsetups.KeeperBenchmarkTestInputs{
			...
			PluginNodeFunding: benchmarkTestNetwork.funding,
...
func getNetworkConfig(networkName string, config *tc.TestConfig) NetworkConfig {
	...
	nc.funding = big.NewFloat(*config.Common.PluginNodeFunding)
...
func SetupAutomationBenchmarkEnv(t *testing.T, keeperTestConfig types.KeeperBenchmarkTestConfig) (*environment.Environment, blockchain.EVMNetwork) {
	...
	pluginResources := performancePluginResources
	if strings.ToLower(keeperTestConfig.GetConfigurationName()) == "soak" {
		pluginResources = soakPluginResources
	...
	for i := 0; i < numberOfNodes; i++ {
		...
		var overrideFn = func(_ interface{}, target interface{}) {
			ctf_config.MustConfigOverridePluginVersion(keeperTestConfig.GetPluginImageConfig(), target)
		...
		cd := plugin.NewWithOverride(i, map[string]any{
			...
			"db":        dbResources,
		}, keeperTestConfig.GetPluginImageConfig(), overrideFn)
