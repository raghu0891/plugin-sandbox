...
import (
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/counter"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/i_automation_registry_master_wrapper_2_3"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/mock_ethusd_aggregator_wrapper"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/weth9_wrapper"

	contractsethereum "github.com/goplugin/pluginv3.0/integration-tests/contracts/ethereum"
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/i_automation_registry_master_wrapper_2_2"
	iregistry22 "github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/i_automation_registry_master_wrapper_2_2"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/generated/i_keeper_registry_master_wrapper_2_1"
...
func PluginK8sClientToPluginNodeWithKeysAndAddress(k8sNodes []*client.PluginK8sClient) []PluginNodeWithKeysAndAddress {
	var nodesAsInterface = make([]PluginNodeWithKeysAndAddress, len(k8sNodes))
...
func PluginClientToPluginNodeWithKeysAndAddress(k8sNodes []*client.PluginClient) []PluginNodeWithKeysAndAddress {
	var nodesAsInterface = make([]PluginNodeWithKeysAndAddress, len(k8sNodes))
	for i, node := range k8sNodes {
		nodesAsInterface[i] = node
	}
...
func (o *EthereumOffchainAggregator) SetConfig(
	pluginNodes []PluginNodeWithKeysAndAddress,
...
func DeployMockPLIETHFeed(client *seth.Client, answer *big.Int) (MockPLIETHFeed, error) {
	abi, err := mock_ethlink_aggregator_wrapper.MockETHPLIAggregatorMetaData.GetAbi()
	if err != nil {
		return &EthereumMockETHPLIFeed{}, fmt.Errorf("failed to get MockPLIETHFeed ABI: %w", err)
	}
	data, err := client.DeployContract(client.NewTXOpts(), "MockPLIETHFeed", *abi, common.FromHex(mock_ethlink_aggregator_wrapper.MockETHPLIAggregatorMetaData.Bin), answer)
	if err != nil {
		return &EthereumMockETHPLIFeed{}, fmt.Errorf("MockPLIETHFeed instance deployment have failed: %w", err)
	}

	instance, err := mock_ethlink_aggregator_wrapper.NewMockETHPLIAggregator(data.Address, wrappers.MustNewWrappedContractBackend(nil, client))
	if err != nil {
		return &EthereumMockETHPLIFeed{}, fmt.Errorf("failed to instantiate MockPLIETHFeed instance: %w", err)
	}
...
func LoadMockPLIETHFeed(client *seth.Client, address common.Address) (MockPLIETHFeed, error) {
	abi, err := mock_ethlink_aggregator_wrapper.MockETHPLIAggregatorMetaData.GetAbi()
	if err != nil {
		return &EthereumMockETHPLIFeed{}, fmt.Errorf("failed to get MockPLIETHFeed ABI: %w", err)
	}
	client.ContractStore.AddABI("MockPLIETHFeed", *abi)
	client.ContractStore.AddBIN("MockPLIETHFeed", common.FromHex(mock_ethlink_aggregator_wrapper.MockETHPLIAggregatorMetaData.Bin))

	instance, err := mock_ethlink_aggregator_wrapper.NewMockETHPLIAggregator(address, wrappers.MustNewWrappedContractBackend(nil, client))
	if err != nil {
		return &EthereumMockETHPLIFeed{}, fmt.Errorf("failed to instantiate MockPLIETHFeed instance: %w", err)
	}
...
// For the integration tests, we also use this ETH/USD feed for PLI/USD feed since they have the same structure
