...
import (
	...
	"github.com/goplugin/pluginv3.0/v2/core/services/ocr2/plugins/ocr2keeper/evmregistry/v21/gasprice"
	...
	for n, rv := range registryVersions {
		...
		t.Run(name, func(t *testing.T) {
			...
			if nodeUpgrade {
				...
				for i := 0; i < 5; i++ {
					err = upgradePluginNodeVersionsLocal(*cfg.GetPluginUpgradeImageConfig().Image, *cfg.GetPluginUpgradeImageConfig().Version, a.DockerEnv.ClCluster.Nodes[i])
...
func setupAutomationTestDocker(
	...
	err = actions.FundPluginNodesFromRootAddress(l, sethClient, contracts.PluginClientToPluginNodeWithKeysAndAddress(env.ClCluster.NodeAPIs()), big.NewFloat(*automationTestConfig.GetCommonConfig().PluginNodeFunding))
