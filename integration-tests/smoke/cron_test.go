...
func TestCronBasic(t *testing.T) {
	...
	require.NoError(t, err, "Creating bridge in plugin node shouldn't fail")

	job, err := env.ClCluster.Nodes[0].API.MustCreateJob(&client.CronJobSpec{
		Schedule:          "CRON_TZ=UTC * * * * * *",
		ObservationSource: client.ObservationSourceSpecBridge(bta),
	})
	require.NoError(t, err, "Creating Cron Job in plugin node shouldn't fail")
...
func TestCronJobReplacement(t *testing.T) {
	...
	require.NoError(t, err, "Recreating Cron Job in plugin node shouldn't fail")
