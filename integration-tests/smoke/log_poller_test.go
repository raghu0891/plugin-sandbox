...
import (
	...
	logpoller "github.com/goplugin/pluginv3.0/integration-tests/universal/log_poller"

	core_logger "github.com/goplugin/pluginv3.0/v2/core/logger"
)

var logScannerSettings = test_env.GetDefaultPluginNodeLogScannerSettingsWithExtraAllowedMessages(testreporters.NewAllowedLogMessage(
	"SLOW SQL QUERY",
...
func TestLogPollerFewFiltersFixedDepth(t *testing.T) {
	executeBasicLogPollerTest(t, test_env.DefaultPluginNodeLogScannerSettings)
}
...
// consistency test that introduces random disruptions by pausing either Plugin or Postgres containers for random interval of 5-20 seconds
...
// HELPER FUNCTIONS
func executeBasicLogPollerTest(t *testing.T, logScannerSettings test_env.PluginNodeLogScannerSettings) {
	...
	t.Cleanup(func() {
		...
		_ = actions.ReturnFundsFromNodes(l, sethClient, contracts.PluginClientToPluginNodeWithKeysAndAddress(testEnv.ClCluster.NodeAPIs()))
	})
	...
	// Start chaos experimnents by randomly pausing random containers (Plugin nodes or their DBs)
...
func executeLogPollerReplay(t *testing.T, consistencyTimeout string) {
	...
	lpTestEnv := prepareEnvironment(l, t, &testConfig, test_env.DefaultPluginNodeLogScannerSettings)
	testEnv := lpTestEnv.testEnv
	sethClient := lpTestEnv.sethClient
...
func prepareEnvironment(l zerolog.Logger, t *testing.T, testConfig *tc.TestConfig, logScannerSettings test_env.PluginNodeLogScannerSettings) logPollerEnvironment {
	cfg := testConfig.LogPoller
