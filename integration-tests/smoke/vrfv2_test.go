...
func TestVRFv2Basic(t *testing.T) {
	...
	t.Run("Canceling Sub And Returning Funds", func(t *testing.T) {
		...
		require.Equal(t, subBalanceLink, subscriptionCanceledEvent.Amount, "SubscriptionCanceled event PLI amount is not equal to sub amount while canceling subscription")
...
func TestVRFOwner(t *testing.T) {
	...
	mockETHLinkFeed, err := env.ContractDeployer.DeployVRFMockETHPLIFeed(big.NewInt(*config.VRFv2.General.LinkNativeFeedResponse))
