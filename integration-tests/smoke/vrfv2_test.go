...
import (
	...
	"github.com/goplugin/pluginv3.0/integration-tests/actions/vrf/vrfv2"
...
func TestVRFv2Basic(t *testing.T) {
	...
	t.Run("Direct Funding", func(t *testing.T) {
		...
		// Check wrapper consumer PLI balance
	...
	t.Run("Oracle Withdraw", func(t *testing.T) {
		...
		require.Equal(
			...
			"PLI funds were not returned after oracle withdraw",
		)
	})
	t.Run("Canceling Sub And Returning Funds", func(t *testing.T) {
		...
		require.Equal(t, subBalanceLink, cancellationEvent.Amount, "SubscriptionCanceled event PLI amount is not equal to sub amount while canceling subscription")
...
func TestVRFOwner(t *testing.T) {
	...
	t.Run("Request Randomness With Force-Fulfill", func(t *testing.T) {
		...
		err = vrfContracts.MockETHPLIFeed.SetBlockTimestampDeduction(big.NewInt(3))
		require.NoError(t, err)
