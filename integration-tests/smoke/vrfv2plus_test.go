...
import (
	...
	vrfcommon "github.com/goplugin/pluginv3.0/integration-tests/actions/vrf/common"
	"github.com/goplugin/pluginv3.0/integration-tests/actions/vrf/vrfv2plus"
...
func TestVRFv2Plus(t *testing.T) {
	...
	newEnvConfig := vrfcommon.NewEnvConfig{
		...
		PluginNodeLogScannerSettings: test_env.DefaultPluginNodeLogScannerSettings,
	}
	...
	t.Run("Canceling Sub And Returning Funds", func(t *testing.T) {
		...
		require.Equal(t, subBalanceLink, cancellationEvent.AmountLink, "SubscriptionCanceled event PLI amount is not equal to sub amount while canceling subscription")
		...
		require.Equal(t, 0, subBalanceLink.Cmp(subFundsReturnedLinkActual), "Returned PLI funds are not equal to sub balance that was cancelled")
	...
	t.Run("Owner Canceling Sub And Returning Funds While Having Pending Requests", func(t *testing.T) {
		...
		//todo - need to use different wallet for each test to verify exact amount of Native/PLI returned
	...
	t.Run("Owner Withdraw", func(t *testing.T) {
		...
		l.Info().
			...
			Msg("Invoking Oracle Withdraw for PLI")

		err = vrfContracts.CoordinatorV2Plus.Withdraw(
			common.HexToAddress(sethClient.MustGetRootKeyAddress().Hex()),
		)
		require.NoError(t, err, "error withdrawing PLI from coordinator to default wallet")
		...
		//not possible to verify exact amount of Native/PLI returned as defaultWallet is used in other tests in parallel which might affect the balance
		...
		require.Equal(t, 1, defaultWalletBalanceLinkAfterWithdraw.Cmp(defaultWalletBalanceLinkBeforeWithdraw), "PLI funds were not returned after oracle withdraw")
	})
}
...
func TestVRFv2PlusMigration(t *testing.T) {
	...
	t.Run("Test migration of Subscription Billing subID", func(t *testing.T) {
		...
		err = newCoordinator.SetPLIAndPLINativeFeed(vrfContracts.LinkToken.Address(), vrfContracts.MockETHPLIFeed.Address())
...
func TestVRFV2PlusWithBHF(t *testing.T) {
	...
	pluginNodeLogScannerSettings := test_env.GetDefaultPluginNodeLogScannerSettingsWithExtraAllowedMessages(testreporters.NewAllowedLogMessage(
		"Pipeline error",
...
func TestVRFv2PlusNodeReorg(t *testing.T) {
	...
	pluginNodeLogScannerSettings := test_env.GetDefaultPluginNodeLogScannerSettingsWithExtraAllowedMessages(
...
func TestVRFv2PlusBatchFulfillmentEnabledDisabled(t *testing.T) {
	...
	t.Run("Batch Fulfillment Enabled", func(t *testing.T) {
		...
		err = actions.DeleteJobs([]*client.PluginClient{vrfNode.CLNode.API})
		require.NoError(t, err)
