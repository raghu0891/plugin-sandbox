...
func TestKeeperBasicSmoke(t *testing.T) {
	...
	for _, rv := range registryVersions {
		...
		t.Run(fmt.Sprintf("registry_1_%d", registryVersion), func(t *testing.T) {
			...
			chainClient, pluginNodes, linkToken, _ := setupKeeperTest(l, t, &config)
			...
			_, err = actions.CreateKeeperJobsLocal(l, pluginNodes, registry, contracts.OCRv2Config{}, fmt.Sprint(chainClient.ChainID))
...
func TestKeeperMigrateRegistry(t *testing.T) {
	...
	_, err = actions.CreateKeeperJobsLocal(l, pluginNodes, secondRegistry, contracts.OCRv2Config{}, fmt.Sprint(chainClient.ChainID))
	require.NoError(t, err, "Error creating keeper jobs")
...
func TestKeeperNodeDown(t *testing.T) {
	...
	for _, rv := range registryVersions {
		...
		t.Run(fmt.Sprintf("registry_1_%d", registryVersion), func(t *testing.T) {
			...
			jobs, err := actions.CreateKeeperJobsLocal(l, pluginNodes, registry, contracts.OCRv2Config{}, fmt.Sprint(chainClient.ChainID))
			require.NoError(t, err, "Error creating keeper jobs")
