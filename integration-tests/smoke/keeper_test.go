...
func TestKeeperBasicSmoke(t *testing.T) {
	...
	for _, rv := range registryVersions {
		...
		t.Run(fmt.Sprintf("registry_1_%d", registryVersion), func(t *testing.T) {
			...
			chainClient, pluginNodes, contractDeployer, linkToken, _ := setupKeeperTest(t, &config)
			...
			_, err = actions.CreateKeeperJobsLocal(l, pluginNodes, registry, contracts.OCRv2Config{}, chainClient.GetChainID().String())
	...
	for _, rv := range registryVersions {
		...
		t.Run(fmt.Sprintf("registry_1_%d", registryVersion), func(t *testing.T) {
			...
			for i := 0; i < len(payees); i++ {
				payees[i], err = pluginNodes[0].PrimaryEthAddress()
...
func TestKeeperMigrateRegistry(t *testing.T) {
	...
	_, err = actions.CreateKeeperJobsLocal(l, pluginNodes, secondRegistry, contracts.OCRv2Config{}, chainClient.GetChainID().String())
	require.NoError(t, err, "Error creating keeper jobs")
	err = chainClient.WaitForEvents()
	require.NoError(t, err, "Error creating keeper jobs")
...
func TestKeeperNodeDown(t *testing.T) {
	...
	for _, rv := range registryVersions {
		...
		t.Run(fmt.Sprintf("registry_1_%d", registryVersion), func(t *testing.T) {
			...
			jobs, err := actions.CreateKeeperJobsLocal(l, pluginNodes, registry, contracts.OCRv2Config{}, chainClient.GetChainID().String())
			require.NoError(t, err, "Error creating keeper jobs")
			err = chainClient.WaitForEvents()
			require.NoError(t, err, "Error creating keeper jobs")
			...
			firstHalfToTakeDown := pluginNodes[:len(pluginNodes)/2+1]
