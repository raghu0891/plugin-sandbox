...
import (
	...
	graphqlClient "github.com/goplugin/pluginv3.0/integration-tests/web/sdk/client"
)
...
func TestRegisteringMultipleJobDistributor(t *testing.T) {
	...
	require.NoError(t, err, "Creating first job distributor in plugin node shouldn't fail")
	...
	require.NoError(t, err, "Creating second job distributor in plugin node shouldn't fail")

	distributors, err := env.ClCluster.Nodes[0].GraphqlAPI.ListJobDistributors(ctx)
	require.NoError(t, err, "Listing job distributors in plugin node shouldn't fail")
