...
type NodeInfo struct {
	CLConfig    clclient.PluginConfig // config to connect to plugin node via API
...
func NewNode(nodeInfo NodeInfo) (*Node, error) {
	...
	pluginClient, err := clclient.NewPluginClient(&nodeInfo.CLConfig, zerolog.Logger{})
	if err != nil {
		return nil, fmt.Errorf("failed to create node rest client: %w", err)
	}
	return &Node{
		gqlClient:  gqlClient,
		restClient: pluginClient,
		Name:       nodeInfo.Name,
...
type Node struct {
	...
	restClient  *clclient.PluginClient // rest client to interact with the node
