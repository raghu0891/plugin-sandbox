...
import (
	...
	"github.com/goplugin/pluginv3.0/integration-tests/ccip-tests/testsetups"
...
// The test environment is then used to start plugin nodes
func CreateDockerEnv(t *testing.T) (
	...
	*test_env.CLClusterTestEnv,
	...
	// so that the plugin nodes and rmn nodes can internally connect to the chain
...
// StartPluginNodes starts docker containers for plugin nodes on the existing test environment based on provided test config
// Once the nodes starts, it updates the devenv EnvironmentConfig with the node info
// which includes plugin API URL, email, password and internal IP
func StartPluginNodes(
	t *testing.T,
	...
	for i := 1; i <= noOfNodes; i++ {
		...
		ccipNode, err := test_env.NewClNode(
			...
			pointer.GetString(cfg.GetPluginImageConfig().Image),
			pointer.GetString(cfg.GetPluginImageConfig().Version),
			toml,
			env.LogStream,
			test_env.WithPgDBOptions(
				ctftestenv.WithPostgresImageVersion(pointer.GetString(cfg.GetPluginImageConfig().PostgresVersion)),
			),
		)
		if err != nil {
			return err
	...
	for i, n := range env.ClCluster.Nodes {
		nodeInfo[i].CLConfig = clclient.PluginConfig{
			URL:        n.API.URL(),
			Email:      n.UserEmail,
...
// FundNodes sends funds to the plugin nodes based on the provided test config
// It also sets up a clean-up function to return the funds back to the deployer account once the test is done
// It assumes that the plugin nodes are already started and the account addresses for all chains are available
func FundNodes(t *testing.T, lggr zerolog.Logger, env *test_env.CLClusterTestEnv, cfg tc.TestConfig, nodes []Node) {
	...
	t.Cleanup(func() {
		for i := range evmNetworks {
			...
			var keyExporters []contracts.PluginKeyExporter
			for j := range nodes {
				node := nodes[j]
	...
	for i := range evmNetworks {
		...
		for _, node := range nodes {
			...
			amount := big.NewFloat(pointer.GetFloat64(cfg.Common.PluginNodeFunding))
...
// RestartPluginNodes restarts the plugin nodes in the test environment
func RestartPluginNodes(t *testing.T, env *test_env.CLClusterTestEnv) error {
