...
import (
	...
	v2toml "github.com/goplugin/pluginv3.0/v2/core/chains/evm/config/toml"
...
func NewNode(
	...
	cfg, db := heavyweight.FullTestDBNoFixturesV2(t, func(c *plugin.Config, s *plugin.Secrets) {
	...
	initOps := []plugin.CoreRelayerChainInitFunc{plugin.InitEVM(context.Background(), relayerFactory, evmOpts)}
	rci, err := plugin.NewCoreRelayerChainInteroperators(initOps...)
...
func CreateKeys(t *testing.T,
	app plugin.Application, chains map[uint64]EVMChain) Keys {
	ctx := tests.Context(t)
