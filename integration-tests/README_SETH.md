...
[Seth](https://github.com/goplugin/plugin-testing-framework/tree/main/seth) is the Ethereum client we use for integration tests. It is designed to be a thin wrapper over `go-ethereum` client that adds a couple of key features:
* key management
...
#### Network is already defined in [known_networks.go](https://github.com/goplugin/plugin-testing-framework/blob/main/networks/known_networks.go)
...
#### It's a new network
Apart from above-mentioned fields you either need to add the network to `known_networks.go` file in the [CTF](https://github.com/goplugin/plugin-testing-framework) or define it in your test TOML file. 
...
You can find a sample `Seth.toml` file [here](https://github.com/goplugin/plugin-testing-framework/tree/main/seth/blob/master/seth.toml). Currently, you cannot use your test TOML file as a Seth configuration file, but we will add ability that in the future.
...
```bash
git clone https://github.com/goplugin/plugin-testing-framework/tree/main/seth
```
...
You can find sample code for doing that for PLI token in [actions](./actions/actions.go) file as `SendLinkFundsToDeploymentAddresses()` method.
