...
import (
	...
	"github.com/goplugin/plugin-testing-framework/lib/k8s/pkg/helm/foundry"
	...
	"github.com/goplugin/plugin-testing-framework/lib/k8s/pkg/helm/reorg"

	"github.com/goplugin/plugin-common/pkg/config"
	k8config "github.com/goplugin/plugin-testing-framework/lib/k8s/config"

	"github.com/goplugin/pluginv3.0/integration-tests/ccip-tests/actions"
	"github.com/goplugin/pluginv3.0/integration-tests/ccip-tests/types/config/node"
	"github.com/goplugin/pluginv3.0/integration-tests/docker/test_env"
	integrationnodes "github.com/goplugin/pluginv3.0/integration-tests/types/config/node"
	evmcfg "github.com/goplugin/pluginv3.0/v2/core/chains/evm/config/toml"
	coreplugin "github.com/goplugin/pluginv3.0/v2/core/services/plugin"
)
...
func SetNodeConfig(nets []blockchain.EVMNetwork, nodeConfig, commonChain string, configByChain map[string]string) (*coreplugin.Config, string, error) {
	var tomlCfg *coreplugin.Config
	var err error
...
func PluginPropsForUpdate(
	t *testing.T,
	testInputs *CCIPTestConfig,
) (map[string]any, int) {
	updateProps := make(map[string]any)
	upgradeImage := pointer.GetString(testInputs.EnvInput.NewCLCluster.Common.PluginUpgradeImage.Image)
	upgradeTag := pointer.GetString(testInputs.EnvInput.NewCLCluster.Common.PluginUpgradeImage.Version)
	...
	if len(testInputs.EnvInput.NewCLCluster.Nodes) > 0 {
		...
		for _, clNode := range testInputs.EnvInput.NewCLCluster.Nodes {
			...
			upgradeImage = pointer.GetString(clNode.PluginUpgradeImage.Image)
			upgradeTag = pointer.GetString(clNode.PluginUpgradeImage.Version)
		...
		updateProps["plugin"] = map[string]interface{}{
...
func PluginChart(
	...
	require.NotNil(t, testInputs.EnvInput.NewCLCluster.Common, "Plugin Common config is not specified")
	...
	clProps["plugin"] = map[string]interface{}{
		"resources": SetResourceProfile(testInputs.EnvInput.NewCLCluster.NodeCPU, testInputs.EnvInput.NewCLCluster.NodeMemory),
		"image": map[string]any{
			"image":   pointer.GetString(testInputs.EnvInput.NewCLCluster.Common.PluginImage.Image),
			"version": pointer.GetString(testInputs.EnvInput.NewCLCluster.Common.PluginImage.Version),
		},
	}
	...
	if len(testInputs.EnvInput.NewCLCluster.Nodes) > 0 {
		...
		for _, clNode := range testInputs.EnvInput.NewCLCluster.Nodes {
			...
			nodesMap = append(nodesMap, map[string]any{
				...
				"plugin": map[string]any{
					"image": map[string]any{
						"image":   pointer.GetString(clNode.PluginImage.Image),
						"version": pointer.GetString(clNode.PluginImage.Version),
					},
				},
				"db": map[string]any{
					"image": map[string]any{
		...
		if len(testInputs.EnvInput.NewCLCluster.Nodes) > 0 {
			for _, clNode := range testInputs.EnvInput.NewCLCluster.Nodes {
				...
				ccipNode, err := test_env.NewClNode(
					...
					pointer.GetString(clNode.PluginImage.Image),
					pointer.GetString(clNode.PluginImage.Version),
					toml,
					env.LogStream,
			...
			for i := 0; i < noOfNodes; i++ {
				...
				ccipNode, err := test_env.NewClNode(
					[]string{env.DockerNetwork.Name},
					pointer.GetString(testInputs.EnvInput.NewCLCluster.Common.PluginImage.Image),
					pointer.GetString(testInputs.EnvInput.NewCLCluster.Common.PluginImage.Version),
					toml,
					env.LogStream,
					test_env.WithPgDBOptions(
...
// UpgradeNodes restarts plugin nodes in the given range with upgrade image
...
func UpgradeNodes(
	...
	if pointer.GetBool(testInputs.TestGroupInput.LocalCluster) {
		...
		for i, clNode := range env.ClCluster.Nodes {
			...
			if len(testInputs.EnvInput.NewCLCluster.Nodes) > 0 {
				if i < len(testInputs.EnvInput.NewCLCluster.Nodes) {
					upgradeImage = pointer.GetString(testInputs.EnvInput.NewCLCluster.Nodes[i].PluginUpgradeImage.Image)
					upgradeTag = pointer.GetString(testInputs.EnvInput.NewCLCluster.Nodes[i].PluginUpgradeImage.Version)
				}
			}
			if upgradeImage == "" || upgradeTag == "" {
				continue
		...
		props, noOfNodesToUpgrade := PluginPropsForUpdate(t, testInputs)
...
func DeployEnvironments(
	...
	err = testEnvironment.
		AddHelm(PluginChart(t, testInputs, nets)).
		Run()
	require.NoError(t, err)
	return testEnvironment
}
