...
import (
	...
	integrationactions "github.com/goplugin/pluginv3.0/integration-tests/actions"
	...
	ccipconfig "github.com/goplugin/pluginv3.0/integration-tests/ccip-tests/testconfig"
...
func (c *CCIPTestConfig) CLClusterNeedsUpgrade() bool {
	...
	if c.EnvInput.NewCLCluster.Common != nil && c.EnvInput.NewCLCluster.Common.PluginUpgradeImage != nil {
		return true
	}
	for _, node := range c.EnvInput.NewCLCluster.Nodes {
		if node.PluginUpgradeImage != nil {
			return true
		}
	}
	return false
	...
	if simulated && len(c.SelectedNetworks) < c.TestGroupInput.NoOfNetworks {
		...
		for i := 0; i < c.TestGroupInput.NoOfNetworks-actualNoOfNetworks; i++ {
			...
			c.SelectedNetworks = append(c.SelectedNetworks, blockchain.EVMNetwork{
				...
				PluginTransactionLimit: n.PluginTransactionLimit,
				Timeout:                   n.Timeout,
	...
	if configureCLNode {
		ccipEnv.CLNodeWithKeyReady.Go(func() error {
			...
			if !o.Cfg.ExistingCLCluster() {
				...
				if err != nil {
					return fmt.Errorf("error connecting to plugin nodes: %w", err)
				}
				...
				if err != nil {
					return fmt.Errorf("error deploying and connecting to plugin nodes: %w", err)
				}
			}
