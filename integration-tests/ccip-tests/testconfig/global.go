...
type Common struct {
	...
	ExistingCLCluster       *CLCluster                                  `toml:",omitempty"` // ExistingCLCluster is the existing plugin cluster to use, if specified it will be used instead of creating a new one
	Mockserver              *string                                     `toml:",omitempty"`
	NewCLCluster            *PluginDeployment                        `toml:",omitempty"` // NewCLCluster is the new plugin cluster to create, if specified along with ExistingCLCluster this will be ignored
...
func (p *Common) ReadFromEnvVar() error {
	...
	pluginImage := ctfconfig.MustReadEnvVar_String(ctfconfig.E2E_TEST_PLUGIN_IMAGE_ENV)
	if pluginImage != "" {
		if p.NewCLCluster == nil {
			p.NewCLCluster = &PluginDeployment{}
		}
		if p.NewCLCluster.Common == nil {
			p.NewCLCluster.Common = &Node{}
		}
		if p.NewCLCluster.Common.PluginImage == nil {
			p.NewCLCluster.Common.PluginImage = &ctfconfig.PluginImageConfig{}
		}

		logger.Debug().Msgf("Using %s env var to override NewCLCluster.Common.PluginImage.Image", ctfconfig.E2E_TEST_PLUGIN_IMAGE_ENV)
		p.NewCLCluster.Common.PluginImage.Image = &pluginImage
	}

	pluginVersion := ctfconfig.MustReadEnvVar_String(ctfconfig.E2E_TEST_PLUGIN_VERSION_ENV)
	if pluginVersion != "" {
		if p.NewCLCluster == nil {
			p.NewCLCluster = &PluginDeployment{}
		}
		...
		logger.Debug().Msgf("Using %s env var to override NewCLCluster.Common.PluginImage.Version", ctfconfig.E2E_TEST_PLUGIN_VERSION_ENV)
		p.NewCLCluster.Common.PluginImage.Version = &pluginVersion
	}

	pluginPostgresVersion := ctfconfig.MustReadEnvVar_String(ctfconfig.E2E_TEST_PLUGIN_POSTGRES_VERSION_ENV)
	if pluginPostgresVersion != "" {
		...
		logger.Debug().Msgf("Using %s env var to override NewCLCluster.Common.PluginImage.PostgresVersion", ctfconfig.E2E_TEST_PLUGIN_POSTGRES_VERSION_ENV)
		p.NewCLCluster.Common.PluginImage.PostgresVersion = &pluginPostgresVersion
	}

	pluginUpgradeImage := ctfconfig.MustReadEnvVar_String(ctfconfig.E2E_TEST_PLUGIN_UPGRADE_IMAGE_ENV)
	if pluginUpgradeImage != "" {
		...
		if p.NewCLCluster.Common.PluginUpgradeImage == nil {
			p.NewCLCluster.Common.PluginUpgradeImage = &ctfconfig.PluginImageConfig{}
		}

		logger.Debug().Msgf("Using %s env var to override NewCLCluster.Common.PluginUpgradeImage.Image", ctfconfig.E2E_TEST_PLUGIN_UPGRADE_IMAGE_ENV)
		p.NewCLCluster.Common.PluginUpgradeImage.Image = &pluginUpgradeImage
	}

	pluginUpgradeVersion := ctfconfig.MustReadEnvVar_String(ctfconfig.E2E_TEST_PLUGIN_UPGRADE_VERSION_ENV)
	if pluginUpgradeVersion != "" {
		...
		logger.Debug().Msgf("Using %s env var to override NewCLCluster.Common.PluginUpgradeImage.Version", ctfconfig.E2E_TEST_PLUGIN_UPGRADE_VERSION_ENV)
		p.NewCLCluster.Common.PluginUpgradeImage.Version = &pluginUpgradeVersion
	}
...
func (p *Common) Validate() error {
	...
	if p.NewCLCluster == nil && p.ExistingCLCluster == nil {
		return errors.New("no plugin or existing cluster specified")
	}
	...
	if p.ExistingCLCluster != nil {
		if err := p.ExistingCLCluster.Validate(); err != nil {
			return fmt.Errorf("error validating existing plugin cluster config %w", err)
		}
		if p.Mockserver == nil {
			return errors.New("no mockserver specified for existing plugin cluster")
		}
		log.Warn().Msg("Using existing plugin cluster, overriding new plugin cluster config if specified")
		p.NewCLCluster = nil
	} else {
		if p.NewCLCluster != nil {
			if err := p.NewCLCluster.Validate(); err != nil {
				return fmt.Errorf("error validating plugin config %w", err)
			}
		}
	}
	return nil
...
func (p *Common) GetPluginImageConfig() *ctfconfig.PluginImageConfig {
	return p.NewCLCluster.Common.PluginImage
}
...
type CLCluster struct {
	...
	NodeConfigs []*client.PluginConfig `toml:",omitempty"`
}

func (c *CLCluster) Validate() error {
	if c.NoOfNodes == nil || len(c.NodeConfigs) == 0 {
		return fmt.Errorf("no plugin nodes specified")
	}
...
func (c *PluginDeployment) Validate() error {
	if c.Common == nil {
		return errors.New("common config can't be empty")
	}
	if c.Common.PluginImage == nil {
		return errors.New("plugin image can't be empty")
	}
	if err := c.Common.PluginImage.Validate(); err != nil {
		return err
	}
	...
	if c.NoOfNodes == nil {
		return errors.New("plugin config is invalid, NoOfNodes should be specified")
	}
	if c.Nodes != nil && len(c.Nodes) > 0 {
		noOfNodes := pointer.GetInt(c.NoOfNodes)
		if noOfNodes != len(c.Nodes) {
			return errors.New("plugin config is invalid, NoOfNodes and Nodes length mismatch")
		}
		for i := range c.Nodes {
			// merge common config with node specific config
			c.Nodes[i].Merge(c.Common)
			node := c.Nodes[i]
			if node.PluginImage == nil {
				return fmt.Errorf("node %s: plugin image can't be empty", node.Name)
			}
			if err := node.PluginImage.Validate(); err != nil {
...
type Node struct {
	...
	PluginImage         *ctfconfig.PluginImageConfig `toml:"PluginImage"`
	PluginUpgradeImage  *ctfconfig.PluginImageConfig `toml:"PluginUpgradeImage"`
...
func (n *Node) Merge(from *Node) {
	...
	if n.PluginImage == nil {
		if from.PluginImage != nil {
			n.PluginImage = &ctfconfig.PluginImageConfig{
				Image:   from.PluginImage.Image,
				Version: from.PluginImage.Version,
			}
		}
	} else {
		if n.PluginImage.Image == nil && from.PluginImage != nil {
			n.PluginImage.Image = from.PluginImage.Image
		}
		if n.PluginImage.Version == nil && from.PluginImage != nil {
			n.PluginImage.Version = from.PluginImage.Version
		}
	}
	// merge upgrade image only if the nodes is marked as NeedsUpgrade to true
	if pointer.GetBool(n.NeedsUpgrade) {
		if n.PluginUpgradeImage == nil {
			if from.PluginUpgradeImage != nil {
				n.PluginUpgradeImage = &ctfconfig.PluginImageConfig{
					Image:   from.PluginUpgradeImage.Image,
					Version: from.PluginUpgradeImage.Version,
				}
			}
		} else {
			if n.PluginUpgradeImage.Image == nil && from.PluginUpgradeImage != nil {
				n.PluginUpgradeImage.Image = from.PluginUpgradeImage.Image
			}
			if n.PluginUpgradeImage.Version == nil && from.PluginUpgradeImage != nil {
				n.PluginUpgradeImage.Version = from.PluginUpgradeImage.Version
			}
		}
	}
