...
The json schema is specified in https://github.com/goplugin/ccip/blob/ccip-develop/integration-tests/ccip-tests/contracts/laneconfig/parse_contracts.go#L96
...
- **NewCLCluster**: [CCIP.Env.NewCLCluster](#ccipenvnewclcluster) - This is mandatory if the test needs to deploy Plugin nodes.
- **ExistingCLCluster**: [CCIP.Env.ExistingCLCluster](#ccipenvexistingclcluster) - This is mandatory if the test needs to run on existing Plugin nodes to deploy ccip jobs.
...
The NetworkConfig is imported from https://github.com/goplugin/plugin-testing-framework/blob/main/config/network.go#L39
...
The name of the networks are taken from [known_networks](https://github.com/goplugin/plugin-testing-framework/blob/main/networks/known_networks.go#L884) in plugin-testing-framework
...
The EVMNetwork is imported from [EVMNetwork](https://github.com/goplugin/plugin-testing-framework/blob/main/blockchain/config.go#L43) in plugin-testing-framework.

If `CCIP.Env.Network.EVMNetworks` config is not set for a network name specified under `CCIP.Env.Networks.selected_networks`, test will try to find the corresponding network config from defined networks in `MappedNetworks` under [known_networks.go](https://github.com/goplugin/plugin-testing-framework/blob/main/networks/known_networks.go)
...
The AnvilConfig is imported from [AnvilConfig](https://github.com/goplugin/plugin-testing-framework/blob/main/config/network.go#L20) in plugin-testing-framework.
...
The NewCLCluster config holds the overall deployment configuration for Plugin nodes.

#### CCIP.Env.NewCLCluster.NoOfNodes

Specifies the number of Plugin nodes to be deployed.

#### CCIP.Env.NewCLCluster.Common

Specifies the common configuration for all Plugin nodes if they share the same configuration.
...
Configuration for the Plugin image.

##### PluginUpgradeImage

Configuration for the Plugin upgrade image. It is used when the node needs an upgrade.

##### BaseConfigTOML

String containing the base configuration toml content for the Plugin node config.

##### CommonChainConfigTOML

String containing the common chain configuration toml content for all EVMNodes in plugin node config.

##### ChainConfigTOMLByChain

String containing the chain-specific configuration toml content for individual EVMNodes in plugin node config. This is keyed by chain ID.

##### DBImage

Database image for the Plugin node.

##### DBTag

Database tag/version for the Plugin node.
...
Specifies the memory to be allocated for each Plugin node. This is valid only if the deployment is on Kubernetes.

#### CCIP.Env.NewCLCluster.NodeCPU

Specifies the CPU to be allocated for each Plugin node. This is valid only if the deployment is on Kubernetes.
...
The ExistingCLCluster config holds the overall connection configuration for existing Plugin nodes.
It is needed when the tests are to be run on Plugin nodes already deployed on some environment.
If this is specified, test will not need to connect to k8 namespace using [CCIP.Env.EnvToConnect](#ccipenvenvtoconnect).
Test can directly connect to the existing Plugin nodes using node credentials without knowing the k8 namespace details.

#### CCIP.Env.ExistingCLCluster.Name

Specifies the name of the existing Plugin cluster. This is used to identify the cluster in the test.

#### CCIP.Env.ExistingCLCluster.NoOfNodes

Specifies the number of Plugin nodes in the existing cluster.

#### CCIP.Env.ExistingCLCluster.NodeConfigs

Specifies the configuration for individual nodes in the existing cluster. Each node config contains the following fields to connect to the Plugin node:

##### CCIP.Env.ExistingCLCluster.NodeConfigs.URL

The URL of the Plugin node.

##### CCIP.Env.ExistingCLCluster.NodeConfigs.Email

The username/email of the Plugin node credential.

##### CCIP.Env.ExistingCLCluster.NodeConfigs.Password

The password of the Plugin node credential.

##### CCIP.Env.ExistingCLCluster.NodeConfigs.InternalIP

The internal IP of the Plugin node.
...
Specifies the logging configuration for the test. Imported from [LoggingConfig](https://github.com/goplugin/plugin-testing-framework/blob/main/config/logging.go#L11) in plugin-testing-framework.
Example usage:
...
Specifies whether commit and execution jobs are to be run on the same Plugin node.
...
Specified the native token funding for each Plugin node. It assumes that the native token decimals is 18.
