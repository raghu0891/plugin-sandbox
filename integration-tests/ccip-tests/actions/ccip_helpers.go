...
import (
	...
	"github.com/goplugin/pluginv3.0/integration-tests/ccip-tests/contracts/laneconfig"
	"github.com/goplugin/pluginv3.0/integration-tests/ccip-tests/testconfig"
	"github.com/goplugin/pluginv3.0/integration-tests/ccip-tests/testreporters"
	testutils "github.com/goplugin/pluginv3.0/integration-tests/ccip-tests/utils"
	...
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/ccip/generated/evm_2_evm_offramp"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/ccip/generated/evm_2_evm_onramp"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/ccip/generated/evm_2_evm_onramp_1_2_0"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/ccip/generated/fee_quoter"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/ccip/generated/mock_rmn_contract"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/ccip/generated/rmn_contract"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/ccip/generated/router"
	"github.com/goplugin/pluginv3.0/v2/core/gethwrappers/ccip/generated/token_pool"
...
type CCIPTestEnv struct {
	...
	CLNodes                  []*client.PluginK8sClient
	...
	CLNodeWithKeyReady       *errgroup.Group // denotes if keys are created in plugin node and ready to be used for job creation
}
...
func (c *CCIPTestEnv) ConnectToExistingNodes(envConfig *testconfig.Common) error {
	...
	for i := 0; i < noOfNodes; i++ {
		...
		clClient, err := client.NewPluginK8sClient(cfg, cfg.InternalIP, namespace)
		if err != nil {
			return fmt.Errorf("failed to create plugin client: %w for node %d config %v", err, i+1, cfg)
		}
...
func (c *CCIPTestEnv) ConnectToDeployedNodes() error {
	if c.LocalCluster != nil {
		...
		for _, pluginNode := range c.LocalCluster.ClCluster.Nodes {
			c.nodeMutexes = append(c.nodeMutexes, &sync.Mutex{})
			c.CLNodes = append(c.CLNodes, &client.PluginK8sClient{
				PluginClient: pluginNode.API,
			})
		}
	} else {
		// in case of k8s, we need to connect to the plugin nodes
		log.Info().Msg("Connecting to launched resources")
		pluginK8sNodes, err := client.ConnectPluginNodes(c.K8Env)
		if err != nil {
			return fmt.Errorf("failed to connect to plugin nodes: %w", err)
		}
		if len(pluginK8sNodes) == 0 {
			return fmt.Errorf("no CL node found")
		}

		for range pluginK8sNodes {
			c.nodeMutexes = append(c.nodeMutexes, &sync.Mutex{})
		}
		c.CLNodes = pluginK8sNodes
...
func (c *CCIPTestEnv) SetUpNodeKeysAndFund(
	...
	if c.CLNodes == nil || len(c.CLNodes) == 0 {
		return fmt.Errorf("no plugin nodes to setup")
	}
	var pluginNodes []*client.PluginClient
	for _, node := range c.CLNodes {
		pluginNodes = append(pluginNodes, node.PluginClient)
	}
	...
	populateKeys := func(chain blockchain.EVMClient) error {
		...
		_, clNodes, err := client.CreateNodeKeysBundle(pluginNodes, "evm", chain.GetChainID().String())
		if err != nil {
	...
	fund := func(ec blockchain.EVMClient) error {
		...
		log.Info().Str("chain id", c1.GetChainID().String()).Msg("Funding Plugin nodes for chain")
		for i := 1; i < len(pluginNodes); i++ {
			cl := pluginNodes[i]
			m := c.nodeMutexes[i]
...
// SetMockserverWithTokenPriceValue sets the mock responses in mockserver that are read by plugin nodes
