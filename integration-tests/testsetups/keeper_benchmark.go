...
// KeeperBenchmarkTest builds a test to check that plugin nodes are able to upkeep a specified amount of Upkeep
...
type KeeperBenchmarkTest struct {
	...
	ethFeed   contracts.MockETHPLIFeed
...
type KeeperBenchmarkTestInputs struct {
	...
	PluginNodeFunding   *big.Float                       // Amount of ETH to fund each plugin node with
...
func (k *KeeperBenchmarkTest) Setup(env *environment.Environment, config tt.KeeperBenchmarkTestConfig) {
	...
	k.pluginNodes, err = client.ConnectPluginNodes(k.env)
	require.NoError(k.t, err, "Connecting to plugin nodes shouldn't fail")
	k.chainClient.ParallelTransactions(true)

	if len(inputs.RegistryVersions) > 1 && !inputs.ForceSingleTxnKey {
		for nodeIndex, node := range k.pluginNodes {
	...
	if common.IsHexAddress(c.LinkTokenAddress) {
		...
		require.NoError(k.t, err, "Failed waiting for PLI Contract deployment")
	}

	if common.IsHexAddress(c.EthFeedAddress) {
		k.ethFeed, err = k.contractDeployer.LoadETHPLIFeed(common.HexToAddress(c.EthFeedAddress))
		require.NoError(k.t, err, "Loading ETH-Link feed Contract shouldn't fail")
	} else {
		k.ethFeed, err = k.contractDeployer.DeployMockETHPLIFeed(big.NewInt(2e18))
	...
	for index := range keysToFund {
		// Fund plugin nodes
		nodesToFund := k.pluginNodes
		if inputs.RegistryVersions[index] == ethereum.RegistryVersion_2_0 || inputs.RegistryVersions[index] == ethereum.RegistryVersion_2_1 {
			nodesToFund = k.pluginNodes[1:]
		}
		err = actions.FundPluginNodesAddress(nodesToFund, k.chainClient, k.Inputs.PluginNodeFunding, index)
		require.NoError(k.t, err, "Funding Plugin nodes shouldn't fail")
	}
...
func (k *KeeperBenchmarkTest) Run() {
	...
	nodesWithoutBootstrap := k.pluginNodes[1:]
	...
	for rIndex := range k.keeperRegistries {
		...
		if inputs.RegistryVersions[rIndex] == ethereum.RegistryVersion_2_0 || inputs.RegistryVersions[rIndex] == ethereum.RegistryVersion_2_1 {
			actions.CreateOCRKeeperJobs(k.t, k.pluginNodes, k.keeperRegistries[rIndex].Address(), k.chainClient.GetChainID().Int64(), txKeyId, inputs.RegistryVersions[rIndex])
			...
			actions.CreateKeeperJobsWithKeyIndex(k.t, k.pluginNodes, k.keeperRegistries[rIndex], txKeyId, ocrConfig, k.chainClient.GetChainID().String())
		}
		err = k.chainClient.WaitForEvents()
	...
	for _, pluginNode := range k.pluginNodes {
		txData, err := pluginNode.MustReadTransactionAttempts()
		if err != nil {
			k.log.Error().Err(err).Msg("Error reading transaction attempts from Plugin Node")
		}
		k.TestReporter.AttemptedPluginTransactions = append(k.TestReporter.AttemptedPluginTransactions, txData)
	}

	k.TestReporter.Summary.Config.Plugin, err = k.env.ResourcesSummary("app=plugin-0")
	if err != nil {
		k.log.Error().Err(err).Msg("Error getting resource summary of plugin node")
	}
	...
	for rIndex := range k.keeperRegistries {
		if inputs.DeleteJobsOnEnd {
			// Delete keeper jobs on plugin nodes
			actions.DeleteKeeperJobsWithId(k.t, k.pluginNodes, rIndex+1)
		}
	}
...
func (k *KeeperBenchmarkTest) TearDownVals(t *testing.T) (
	*testing.T,
	...
	return t, k.namespace, k.pluginNodes, &k.TestReporter, k.testConfig, k.chainClient
}
...
func (k *KeeperBenchmarkTest) ensureInputValues() {
	...
	require.NotNil(k.t, k.Inputs.PluginNodeFunding, "You need to set a funding amount for plugin nodes")
	clFunds, _ := k.Inputs.PluginNodeFunding.Float64()
	require.GreaterOrEqual(k.t, clFunds, 0.0, "Expecting Plugin node funding to be more than 0 ETH")
...
func (k *KeeperBenchmarkTest) SendSlackNotification(slackClient *slack.Client, config tt.KeeperBenchmarkTestConfig) error {
	...
	formattedDashboardUrl := fmt.Sprintf("%s%s?from=%d&to=%s&var-namespace=%s&var-cl_node=plugin-0-0", grafanaUrl, dashboardUrl, k.TestReporter.Summary.StartTime, "now", k.env.Cfg.Namespace)
...
func (k *KeeperBenchmarkTest) DeployBenchmarkKeeperContracts(index int) {
	...
	if registryVersion <= ethereum.RegistryVersion_1_3 { // Legacy keeper - v1.X
		...
		// Fund the registry with 1 PLI * amount of AutomationConsumerBenchmark contracts
		...
		err := k.linkToken.Transfer(registry.Address(), big.NewInt(0).Mul(big.NewInt(1e18), big.NewInt(int64(k.Inputs.Upkeeps.NumberOfUpkeeps))))
		require.NoError(k.t, err, "Funding keeper registry contract shouldn't fail")
		ocrConfig, err := actions.BuildAutoOCR2ConfigVars(k.t, k.pluginNodes[1:], *k.Inputs.KeeperRegistrySettings, registrar.Address(), k.Inputs.DeltaStage)
