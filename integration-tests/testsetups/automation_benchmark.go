...
import (
	...
	reportModel "github.com/goplugin/plugin-testing-framework/lib/testreporters"
	...
	autotestconfig "github.com/goplugin/pluginv3.0/integration-tests/testconfig/automation"
...
func (k *KeeperBenchmarkTest) Setup(env *environment.Environment, config testconfig.TestConfig) {
	...
	for index := range inputs.RegistryVersions {
		...
		a := automationv2.NewAutomationTestK8s(k.log, k.chainClient, k.pluginNodes, &config)
	...
	for index := range keysToFund {
		...
		err = actions.FundPluginNodesAtKeyIndexFromRootAddress(k.log, k.chainClient, contracts.PluginK8sClientToPluginNodeWithKeysAndAddress(nodesToFund), k.Inputs.PluginNodeFunding, index)
...
func (k *KeeperBenchmarkTest) TearDownVals(t *testing.T) (
	...
	*seth.Client,
	...
	return t, k.chainClient, k.namespace, k.pluginNodes, &k.TestReporter, k.testConfig
}
