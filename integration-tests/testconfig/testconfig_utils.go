...
// MissingImageInfoAsError return a helfpul error message when the no Plugin image info is found in TOML config.
// If legacy env vars are found it prints ready to use TOML configuration
func MissingImageInfoAsError(errStr string) error {
	missingImage := `
Plugin image is a secret and must be set as env var in ~/.testsecrets file or passed as env var (either E2E_TEST_PLUGIN_IMAGE or E2E_TEST_PLUGIN_UPGRADE_IMAGE). You might have used old configuration approach.
Please refer to integration-tests/testconfig/README.md for more information.
`
	missingVersion := `
Plugin version must be set in toml config.
`

	if os.Getenv("E2E_TEST_PLUGIN_IMAGE") == "" || os.Getenv("E2E_TEST_PLUGIN_UPGRADE_IMAGE") == "" {
		return fmt.Errorf(fmt.Sprintf("%s\n%s", errStr, missingImage))
	}
	if os.Getenv("PLUGIN_VERSION") == "" || os.Getenv("PLUGIN_UPGRADE_VERSION") == "" {
