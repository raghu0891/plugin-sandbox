...
// PluginConfig represents the variables needed to connect to a Plugin node
...
// ResponseSlice is the generic model that can be used for all Plugin API responses that are an slice
...
// Response is the generic model that can be used for all Plugin API responses
...
// BridgeType is the model that represents the bridge when read or created on a Plugin node
type BridgeType struct {
	Data BridgeTypeData `json:"data"`
}

// BridgeTypeData is the model that represents the bridge when read or created on a Plugin node
type BridgeTypeData struct {
	Attributes BridgeTypeAttributes `json:"attributes"`
}

// Bridges is the model that represents the bridges when read on a Plugin node
type Bridges struct {
	Data []BridgeTypeData `json:"data"`
}

// BridgeTypeAttributes is the model that represents the bridge when read or created on a Plugin node
...
// ExportedEVMKey holds all details needed to recreate a private key of the Plugin node
...
// JobSpec represents the different possible job types that Plugin nodes can handle
...
type OCRTaskJobSpec struct {
	...
	ContractAddress          string             `toml:"contractAddress"`                        // Address of the OCR contract
	EVMChainID               string             `toml:"evmChainID"`
	P2PBootstrapPeers        []*PluginClient `toml:"p2pBootstrapPeers"`  // P2P ID of the bootstrap node
...
// PluginProfileResults holds the results of asking the Plugin node to run a PPROF session
type PluginProfileResults struct {
	Reports                 []*PluginProfileResult
...
// PluginProfileResult contains the result of a single PPROF run
type PluginProfileResult struct {
	Type string
	Data []byte
}

// NewBlankPluginProfileResults returns all the standard types of profile results with blank data
func NewBlankPluginProfileResults() *PluginProfileResults {
	results := &PluginProfileResults{
		Reports: make([]*PluginProfileResult, 0),
	}
	...
	for _, profile := range profileStrings {
		results.Reports = append(results.Reports, &PluginProfileResult{Type: profile})
	}
	return results
}
