		...
		t.Run(name, func(t *testing.T) {
			...
			for name, testCase := range testCases {
				...
				t.Run(fmt.Sprintf("Automation_%s", name), func(t *testing.T) {
					...
					t.Cleanup(func() {
						err := actions.TeardownSuite(t, chainClient, testEnvironment, pluginNodes, nil, zapcore.PanicLevel, &config)
						require.NoError(t, err, "Error tearing down environment")
					})

					a := automationv2.NewAutomationTestK8s(l, chainClient, pluginNodes, &config)
					...
					err = actions.FundPluginNodesFromRootAddress(l, a.ChainClient, contracts.PluginK8sClientToPluginNodeWithKeysAndAddress(pluginNodes[1:]), big.NewFloat(*config.Common.PluginNodeFunding))
