...
type KeeperBenchmarkTestReporter struct {
	...
	AttemptedPluginTransactions []*client.TransactionsData `json:"attemptedPluginTransactions"`
...
func (k *KeeperBenchmarkTestReporter) WriteReport(folderLocation string) error {
	...
	txs, err := json.Marshal(k.AttemptedPluginTransactions)
	if err != nil {
		return err
	}
...
func (k *KeeperBenchmarkTestReporter) SendSlackNotification(t *testing.T, slackClient *slack.Client, grafanaUrlProvider testreporters.GrafanaURLProvider) error {
	...
	formattedDashboardUrl := fmt.Sprintf("%s%s?from=%d&to=%d&var-namespace=%s&var-cl_node=plugin-0-0", grafanaUrl, dashboardUrl, k.Summary.StartTime, k.Summary.EndTime, k.namespace)
	...
	return testreporters.UploadSlackFile(slackClient, slack.FileUploadParameters{
		Title:           fmt.Sprintf("Automation Benchmark Attempted Plugin Txs %s", k.namespace),
		Filetype:        "json",
